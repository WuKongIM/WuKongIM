...
This file is for agents working on `test/e2e/message/request_scoped_subscriber_delivery`.
...
One real three-node cluster started through `test/e2e/suite`. The HTTP API request is sent to node 1 while subscribers are connected through WKProto gateways on other nodes.
