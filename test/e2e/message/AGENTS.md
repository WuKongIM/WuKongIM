...
| `conversation_directory_multi_node` | Prove Channel-Leader-grouped hydration, live committed-head reads, unavailable-Leader isolation with cursor progress, and `/conversation/retry` recovery in a real three-node cluster. | `GOWORK=off go test -tags=e2e ./test/e2e/message/conversation_directory_multi_node -count=1 -timeout 3m -p=1` |
