...
| `easy_sdk_jsonrpc` | Prove source-aligned EasySDK iOS v1.0.3 binary/camelCase and Android v1.0.3 text/snake_case JSON-RPC clients complete bidirectional messaging, acknowledgments, ping correlation, disconnect cleanup, and reconnect through public endpoints on a 256-slot single-node cluster. | `GOWORK=off go test -tags=e2e ./test/e2e/message/easy_sdk_jsonrpc -count=1 -timeout 2m -p=1` |
