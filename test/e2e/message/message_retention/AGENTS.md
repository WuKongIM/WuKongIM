...
cluster-authoritative logical Channel compaction boundary and that enabled
...
```bash
GOWORK=off go test -tags=e2e ./test/e2e/message/message_retention -count=1 -timeout 2m -p=1
```
