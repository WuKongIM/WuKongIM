...
func TestActivatedDynamicNodeDeliversCrossNodeTraffic(t *testing.T) {
	s := suite.New(t)
	const joinToken = "e2e-dynamic-delivery-token"
	...
	userA := connectedDynamicDeliveryClient(t, node4, "e2e-dynamic-delivery-a")
	defer func() { _ = userA.Close() }()
	node2 := cluster.MustNode(2)
	userB := connectedDynamicDeliveryClient(t, node2, "e2e-dynamic-delivery-b")
	defer func() { _ = userB.Close() }()

	sendAndRequireDynamicDelivery(t, cluster, node2, userA, userB, "e2e-dynamic-delivery-a", "e2e-dynamic-delivery-b", 1, "e2e-dynamic-a-to-b-1", []byte("hello b from dynamic node"))
	eventuallySendAndRequireDynamicDelivery(t, cluster, node4, userB, userA, "e2e-dynamic-delivery-b", "e2e-dynamic-delivery-a", "e2e-dynamic-b-to-a", []byte("hello dynamic node from b"), 30*time.Second)
}
