...
const gofailDynamicNodeEnv = "WK_E2E_GOFAIL_DYNAMIC_NODE"

func requireGofailDynamicNodeEnabled(t *testing.T) {
	t.Helper()
	if os.Getenv(gofailDynamicNodeEnv) != "1" {
		t.Skipf("set %s=1 to run gofail dynamic-node e2e tests", gofailDynamicNodeEnv)
	}
	if strings.TrimSpace(os.Getenv("WK_E2E_BINARY")) == "" {
		t.Skip("set WK_E2E_BINARY to a gofail-enabled cmd/wukongim binary")
	}
}
...
func TestGofailDynamicNodeBinaryExposesFailpoints(t *testing.T) {
	...
	cluster := s.StartThreeNodeCluster(
		...
		suite.WithDynamicJoinToken("e2e-gofail-smoke-token"),
