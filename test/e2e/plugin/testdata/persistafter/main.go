...
import (
	...
	pluginv2 "github.com/WuKongIM/WuKongIM/internal/usecase/plugin"
...
const (
	persistAfterFile = "persist_after.jsonl"
	readyFile        = "persist_after.ready"
)
...
func main() {
	...
	if err := run(*socketPath, *sandboxDir); err != nil {
		_, _ = fmt.Fprintf(os.Stderr, "persistafter: %v\n", err)
		os.Exit(1)
	}
}
...
func (c *client) startPlugin(ctx context.Context) error {
	body, err := proto.Marshal(&pluginproto.PluginInfo{
		No:      c.no,
		Name:    "PersistAfter Compatibility Plugin",
		Version: "0.0.1",
...
func (c *client) handleMessage(body []byte) error {
	...
	if msg.MsgType != pluginv2.MsgTypePersistAfter {
		return nil
	}
	var batch pluginproto.MessageBatch
	if err := proto.Unmarshal(msg.Content, &batch); err != nil {
		return err
	}
	return c.appendBatch(batch.GetMessages())
}

func (c *client) appendBatch(messages []*pluginproto.Message) error {
	path := filepath.Join(c.sandbox, persistAfterFile)
	...
	for _, msg := range messages {
		...
		if err := enc.Encode(persistRecord{
