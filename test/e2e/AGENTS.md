...
| --- | --- | --- | --- |
| `backup` | `test/e2e/backup/three_node_restore` | Prove continuous checkpoint/erasure/fresh restore and post-write semantics across Controller, Slot/data, and restore-leader process failures; locally prove single-copy repair and live-source dual-copy rebase, while an opt-in form runs the same drill against production Alibaba OSS/RAM/ObjectWorm with a protected deployment key package. | `GOWORK=off go test -tags=e2e ./test/e2e/backup/three_node_restore -count=1 -timeout 18m -p=1` |
