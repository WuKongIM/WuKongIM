...
| `message` | `test/e2e/message/wukongimv2_conversation_projection` | Prove `cmd/wukongimv2` projects group messages into `/conversation/list` using dense small-group rows and sparse large-group sender rows, with an opt-in 100k sparse/metrics stress path. | `GOWORK=off go test -tags=e2e ./test/e2e/message/wukongimv2_conversation_projection -count=1` |
