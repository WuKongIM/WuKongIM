...
| --- | --- | --- | --- |
| `backup` | `test/e2e/backup/three_node_restore` | Prove a real three-node source publishes a baseline plus incremental point, records a later permanent erasure, and a fresh three-node successor restores the older point without resurrecting or reusing erased sequences before normal traffic continues. | `GOWORK=off go test -tags=e2e ./test/e2e/backup/three_node_restore -count=1 -timeout 8m -p=1` |
