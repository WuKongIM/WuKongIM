...
| `backup` | `test/e2e/backup/scheduled_restore` | Prove authenticated Manager plan enablement publishes an initial 256-Hash-Slot full archive and restores exact point-in-time business state in a real single-node cluster. | `GOWORK=off go test -tags=e2e ./test/e2e/backup/scheduled_restore -count=1 -timeout 5m -p=1` |
