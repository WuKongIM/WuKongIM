...
func TestThreeNodeBackupCompletesWhileDataNodeRemainsOffline(t *testing.T) {
	...
	verificationTask, err := manager.VerifyBackupRestorePoint(verifyCtx, incremental.ID)
	cancelVerify()
	require.NoError(t, err, cluster.DumpDiagnostics())
	...
	verification := manager.WaitBackupVerificationSucceeded(t, verificationTask.ID, 30*time.Second)
