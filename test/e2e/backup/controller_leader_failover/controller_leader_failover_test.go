...
func TestThreeNodeBackupControllerLeaderFailoverResumesIncremental(t *testing.T) {
	...
	newActive, newPublished := newManager.WaitBackupActiveOrPublished(t, triggered.ID, triggered.RestorePointID, 10*time.Second)
	if newActive != nil {
		...
		require.NotNil(t, newPublished)
		require.Equal(t, triggered.RestorePointID, newPublished.ID)
	...
	verificationTask, err := newManager.VerifyBackupRestorePoint(verifyCtx, incremental.ID)
	cancelVerify()
	require.NoError(t, err, cluster.DumpDiagnostics())
	require.NotEmpty(t, verificationTask.ID)
	require.Equal(t, incremental.ID, verificationTask.RestorePointID)
	require.Contains(t, []string{"pending", "running"}, verificationTask.Status)
	verification := newManager.WaitBackupVerificationSucceeded(t, verificationTask.ID, 30*time.Second)
	require.Equal(t, incremental.ID, verification.RestorePointID)
	require.Equal(t, "succeeded", verification.Status)
