...
const (
	lifecycleFile = "lifecycle.jsonl"
	readyFile     = "lifecycle.ready"
)
...
func main() {
	...
	if err := run(*socketPath, *sandboxDir); err != nil {
		_, _ = fmt.Fprintf(os.Stderr, "lifecycle: %v\n", err)
		os.Exit(1)
	}
}
...
func (c *client) startPlugin(ctx context.Context) error {
	body, err := proto.Marshal(&pluginproto.PluginInfo{
		No:      c.no,
		Name:    "Lifecycle Compatibility Plugin",
		Version: "0.0.1",
		Methods: []string{"ConfigUpdate"},
		ConfigTemplate: &pluginproto.ConfigTemplate{Fields: []*pluginproto.Field{
			...
			{Name: "mode", Type: pluginproto.FieldTypeString.String(), Label: "Mode"},
		}},
	})
	if err != nil {
		return err
	...
	config, err := decodeConfig(startup.GetConfig())
	if err != nil {
		return err
	}
	return c.appendRecord(lifecycleRecord{Event: "startup", PID: os.Getpid(), Config: config})
}
...
func (c *client) handleRequest(body []byte) {
	...
	if err := req.Unmarshal(body); err != nil {
		...
		resp.Id = req.Id
		switch req.Path {
		case "/plugin/config_update":
			config, err := decodeConfig(req.Body)
			if err != nil {
				resp.Status = wkrpcproto.StatusError
				resp.Body = []byte(err.Error())
			} else if err := c.appendRecord(lifecycleRecord{Event: "config_update", PID: os.Getpid(), Config: config}); err != nil {
				resp.Status = wkrpcproto.StatusError
				resp.Body = []byte(err.Error())
			}
		case "/stop":
			if err := c.appendRecord(lifecycleRecord{Event: "stop", PID: os.Getpid()}); err != nil {
				resp.Status = wkrpcproto.StatusError
				resp.Body = []byte(err.Error())
			}
		default:
...
func (c *client) appendRecord(record lifecycleRecord) error {
	path := filepath.Join(c.sandbox, lifecycleFile)
