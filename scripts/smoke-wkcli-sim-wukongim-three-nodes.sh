...
AUTO_JOIN_CONFIG_PATH="${WK_WKCLI_SIM_THREE_SMOKE_AUTO_JOIN_CONFIG:-$OUT_DIR/wukongim-node4.conf}"
...
Options:
  ...
  --auto-join-config PATH   Generated joining node config path. Default: OUT_DIR/wukongim-node4.conf.
...
if [[ "$AUTO_JOIN_CONFIG_PATH_SET" -eq 0 ]]; then
  AUTO_JOIN_CONFIG_PATH="$OUT_DIR/wukongim-node${AUTO_JOIN_NODE_ID}.conf"
fi
