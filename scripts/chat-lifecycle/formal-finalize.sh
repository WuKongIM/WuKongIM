...
WK_CHAT_EVIDENCE_TIMEOUT="$remote_timeout" \
  scripts/chat-lifecycle/collect-terminal-evidence.sh formal "$WK_CHAT_FINAL_DIR/evidence" || terminal_evidence_complete=false
...
hold_failure_for_diagnosis() {
  ...
  budget_snapshot="$(scripts/chat-lifecycle/diagnosis-budget.sh \
    ...
    "$WK_CHAT_HANDOFF_DIR/receipt.json" "$now_rfc" \
    "$WK_CHAT_FINAL_DIR/formal/final.json" "$WK_CHAT_FINAL_DIR/capacity/final.json")" || return 0
