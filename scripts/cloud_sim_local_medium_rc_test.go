...
func TestLocalMediumRCRevisionNeutralHarness(t *testing.T) {
	...
	for _, fragment := range []string{
		...
		"Go overlays replace files discovered by the package loader",
		"! -L \"$common_target\"",
		"ln -h \"$source\" \"$target\"",
		"ln -T \"$source\" \"$target\"",
		...
		"require_commands",
		"${TMPDIR:-/tmp}",
		"GOTOOLCHAIN=\"$declared_go_version\"",
		"selected Go toolchain differs from candidate declaration",
		"could not canonicalize work root",
		"GIT_CONFIG_KEY_0=status.showUntrackedFiles",
		"-buildvcs=true",
		"could not reserve output directory",
		"TestLocalMediumRCWorkloadEquivalence",
		"selected_go_root",
