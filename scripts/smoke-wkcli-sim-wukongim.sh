...
usage() {
  cat <<'USAGE'
Usage: scripts/smoke-wkcli-sim-wukongim.sh [options]

Starts a temporary cmd/wukongim single-node cluster with the v2 bench API
...
print_plan() {
  ...
  printf 'node_cmd=env WK_NODE_ID=1 WK_NODE_DATA_DIR=%s WK_CLUSTER_LISTEN_ADDR=%s WK_CLUSTER_NODES=[{\"id\":1,\"addr\":\"%s\"}] WK_API_LISTEN_ADDR=%s WK_BENCH_API_ENABLE=true WK_EXTERNAL_TCPADDR=%s go run ./cmd/wukongim\n' \
...
start_node() {
  local listen
  listen="$(api_listen_addr)"
  log "starting cmd/wukongim: api=${API_ADDR} gateway=${GATEWAY_ADDR} cluster=${CLUSTER_ADDR}"
  (
    cd "$ROOT_DIR"
    env \
      WK_NODE_ID=1 \
