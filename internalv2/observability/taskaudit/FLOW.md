...
`taskaudit` stores bounded ControllerV2 task audit history for local manager
reads. It persists append-only JSONL under the node data directory and keeps a
bounded in-memory projection for list and timeline queries.
...
serializes with append and rewrites only retained events.
