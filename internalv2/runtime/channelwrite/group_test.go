...
func TestSubmitLocalIgnoresCallerCancellationAfterMailboxAcceptance(t *testing.T) {
	...
	group := newStartedTestGroup(t, Options{LocalNodeID: 1, MailboxSize: 1, MessageID: newSequenceIDsForPrepare(1), Appender: appender})
	target := AuthorityTarget{ChannelID: ChannelID{ID: "room", Type: 2}, LeaderNodeID: 1}
...
func TestStopCancelsAcceptedAppendAndClosesAdmission(t *testing.T) {
	appender := newBlockingAppenderForAppendTest()
	group := New(Options{LocalNodeID: 1, MailboxSize: 1, MessageID: newSequenceIDsForPrepare(1), Appender: appender})
	if err := group.Start(context.Background()); err != nil {
		t.Fatalf("Start() error = %v", err)
	}
	target := AuthorityTarget{ChannelID: ChannelID{ID: "room", Type: 2}, LeaderNodeID: 1}
