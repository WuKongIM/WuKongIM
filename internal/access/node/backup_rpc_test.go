...
func TestBackupMessageShardRPCUsesBoundedSourceNodePort(t *testing.T) {
	...
	request := runtimebackup.CaptureRequest{Generation: "generation-1", RebaseEpoch: 4, HashSlot: 1, ConfigFingerprint: strings.Repeat("a", 64)}
...
func TestBackupEvidenceCodecsRejectV1WireShapes(t *testing.T) {
	...
	plan := backupusecase.RestorePlan{
		ID: "plan-1", CheckpointID: "restore-1", CheckpointSHA256: strings.Repeat("a", 64),
