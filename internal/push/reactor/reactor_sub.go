	...
	for {
		...
		if !goption.G.Stress {
			if r.continReadEventCount < 1000 {
				...
				r.Warn("push: too many consecutive ready", zap.Int("continReadEventCount", r.continReadEventCount))
...
func (r *reactorSub) addAction(a reactor.PushAction) bool {
	if goption.G.Stress {
		r.mustAddAction(a)
		return true
	}
	...
	if !added {
		r.Warn("drop push action,queue is full",
...
func (r *reactorSub) mustAddAction(a reactor.PushAction) {
