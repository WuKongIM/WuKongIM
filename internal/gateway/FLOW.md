...
`internal/gateway` 是节点内客户端接入基础设施。它负责监听器绑定、底层传输适配、协议编解码、会话生命周期、认证握手、帧分发、写队列/超时/空闲关闭，以及网关级观测事件。

**负责**:
- TCP / WebSocket 客户端连接接入。
- `wkproto`、`jsonrpc`、`wsmux` 协议适配到统一 `frame.Frame`。
- Session 创建、关闭、写队列、出站限流、空闲检测与请求上下文取消。
...
| `session/` | `session.New` → `Session` | Session 抽象、Session manager、出站 encoded queue、session values |
| `transport/` | `Factory` / `Listener` / `Conn` | 底层传输抽象，约束 Build 顺序、Start/Stop 和 Conn 回调语义 |
| `transport/gnet/` | `gnet.NewFactory` | 默认高性能 transport，多个逻辑 listener 共享 gnet engine，内置 TCP 和 WebSocket handshake/frame 支持 |
| `transport/stdnet/` | `stdnet.NewFactory` | 标准库 transport，便于测试、兼容和回滚 |
...
入口: `internal/app/build.go` → `gateway.New` → `core.NewServer` → `Gateway.Start`
...
app build:
  ...
  ③ gateway.New:
     - buildRegistry 注册 stdnet、gnet、wkproto、jsonrpc、wsmux
...
OnOpen:
  ...
  ④ 创建 session.Session，注入 WriteFrameFn=encodeAndQueue
  ⑤ 注册 state 到 core.states 和 session.Manager
  ⑥ adapter.OnOpen
  ⑦ 无需认证且非 wsmux 时立即 dispatch OnSessionOpen
  ⑧ 启动该 session 的 writer goroutine
```

`gnet` transport 会让同一个 factory.Build 调用中的多个逻辑 listener 共享一个 engine group；`stdnet` transport 为每个 listener 创建独立 TCP 或 WebSocket listener。
...
WriteFrame:
  ...
  ② session.WriteFrame 串行化出站 encode，避免 Close 与 Write 并发破坏队列
  ③ core.encodeAndQueue 调用当前 protocol adapter.Encode
  ...
  ④ 记录 OnFrameOut
  ⑤ 入 session encoded queue，受 WriteQueueSize 和 MaxOutboundBytes 限制
  ⑥ writer goroutine DequeueEncoded 后 writePayload
     - Conn 实现 transport.WriteTimeoutPolicy 时交给 transport 管理写超时
     - 否则优先用 SetWriteDeadline；没有 deadline 能力时使用超时 goroutine 包裹
  ⑦ 写队列满、出站字节超限、写超时会映射到对应 CloseReason
```
...
- `gnet` 是默认 transport，支持 TCP 与 WebSocket，并让多个逻辑 listener 共享一个 engine group。
- `gnet` WebSocket 实现包含 handshake 校验、masked client frame 校验、ping/pong/close、fragment reassembly 和写出 frame 封装。
- `stdnet` 用标准库 `net` / `net/http` / gorilla websocket 实现，适合测试、排障和回滚。
- transport 层只负责 bytes 与 conn lifecycle，不应依赖 gateway handler 之外的业务对象。

## 8. 依赖边界

- `internal/gateway` 可以依赖 `pkg/protocol/*`、`pkg/observability/sendtrace`、`pkg/wklog` 等通用包。
- `internal/gateway` 不应依赖 `internal/access/*`、`internal/usecase/*`、`internal/runtime/*`、`internal/app`。
- `internal/access/gateway` 可以依赖 `internal/gateway`，负责把 `frame.Frame` 转为 message/presence 用例命令。
- Session value key 是 gateway 与 access/gateway 的窄合约；新增或重命名时需要同步 mapper、lifecycle 和测试。
- 新业务帧支持优先加在 `internal/access/gateway`；只有 wire format 或连接语义变化才修改 `internal/gateway`。
...
```bash
GOWORK=off go test ./internal/gateway -count=1
GOWORK=off go test ./internal/gateway/... -count=1
GOWORK=off go test ./internal/access/gateway -count=1
GOWORK=off go test ./internal -run TestInternalImportBoundaries -count=1
```

涉及真实 TCP / WebSocket 行为时，优先补 `internal/gateway` 对应 transport/protocol/core 的单元测试；跨到 message/presence 用例时，补 `internal/access/gateway` 或 `internal/app` 装配测试。配置字段变更必须同步 `cmd/wukongim/config.go`、`internal/app/config.go`、`wukongim.conf.example` 和字段英文注释。
