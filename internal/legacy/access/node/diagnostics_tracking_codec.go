...
func decodeDiagnosticsTrackingRequest(body []byte) (diagnosticsTrackingRequest, error) {
	if len(body) > maxDiagnosticsBodyBytes {
		return diagnosticsTrackingRequest{}, fmt.Errorf("access/node: diagnostics tracking request body too large")
	}
	if !hasMagic(body, diagnosticsTrackingRequestMagic[:]) {
		return diagnosticsTrackingRequest{}, fmt.Errorf("access/node: invalid diagnostics tracking request codec")
	}
	...
	if next != len(body) {
		return diagnosticsTrackingRequest{}, fmt.Errorf("access/node: trailing diagnostics tracking request bytes")
	}
...
func decodeDiagnosticsTrackingResponse(body []byte) (diagnosticsTrackingResponse, error) {
	if len(body) > maxDiagnosticsBodyBytes {
		return diagnosticsTrackingResponse{}, fmt.Errorf("access/node: diagnostics tracking response body too large")
	}
	if !hasMagic(body, diagnosticsTrackingResponseMagic[:]) {
		return diagnosticsTrackingResponse{}, fmt.Errorf("access/node: invalid diagnostics tracking response codec")
	}
	...
	if offset != len(body) {
		return diagnosticsTrackingResponse{}, fmt.Errorf("access/node: trailing diagnostics tracking response bytes")
	}
	return resp, nil
}
...
func readDiagnosticsTrackingRules(body []byte, offset int) ([]diagnostics.TrackingRule, int, error) {
	...
	if count > maxDiagnosticsTrackingRules {
		return nil, offset, fmt.Errorf("access/node: diagnostics tracking rules exceeds limit")
	}
	offset = next
