# internal/legacy 流程文档

> This document describes the former v1 server runtime after it was moved under
> `internal/legacy`. Package paths in the sections below are relative to
> `internal/legacy` unless an absolute repository path is shown. New product
> behavior should use the promoted `internal/{access,usecase,runtime,infra,app}`
> layers instead of extending this legacy tree.
