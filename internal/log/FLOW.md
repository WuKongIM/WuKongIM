...
3. `AppLogReader` performs bounded initial, forward, and contextual reads using
   opaque cursors, folds Go stack continuations into logical events, redacts
   absolute source paths, and caps returned event text.
...
- Reader input cannot select a filesystem path; scans and returned events have
  independent bounds, and returned event data cannot expose absolute Go source
  paths.
- Context reads return exact before/after logical events from the same selected
  file.
