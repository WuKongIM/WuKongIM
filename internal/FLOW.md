...
| `runtime/channelmeta` | `runtime/channelmeta/` | Channel runtime metadata resolver、合约、激活缓存、bootstrap/lease renewal、liveness 与 watcher 原语 |
...
New(Config):
  → build(cfg):
    ...
    ⑧ 创建 ChannelMetaSync runtime resolver（app 只保留 wiring wrapper 与 leader repair adapter）
    ⑨ 创建 Presence:
