...
| `gateway.Gateway` | `gateway/gateway.go` | 网关层：管理 Transport/Protocol/Session，双向实时通信 |
| `access/gateway.Handler` | `access/gateway/handler.go` | 网关桥接：将 Frame 路由到 Usecase 层 |
| `access/api.Server` | `access/api/server.go` | HTTP API：服务端消息发送、频道消息同步、Token 管理、会话同步、会话未读变更、健康检查 |
| `access/node.Client` | `access/node/options.go` | 节点间 RPC 客户端：投递提交/推送/确认、Presence、会话事实、channel leader repair/evaluate |
| `access/node.Adapter` | `access/node/options.go` | 节点间 RPC 服务端：注册到集群 RPCMux，处理入站 RPC（含 channel leader repair/evaluate） |
| `contracts/messageevents` | `contracts/messageevents/events.go` | 已提交消息事件合约 |
| `contracts/deliveryevents` | `contracts/deliveryevents/events.go` | 投递回执与会话关闭事件合约 |
| `usecase/message.App` | `usecase/message/app.go` | 消息用例：发送、接收确认、会话关闭处理 |
| `usecase/delivery.App` | `usecase/delivery/app.go` | 投递用例：提交投递、确认、离线处理 |
| `usecase/presence.App` | `usecase/presence/app.go` | 在线状态用例：激活/去激活、权威路由注册、心跳 |
| `usecase/conversation.App` | `usecase/conversation/app.go` | 会话用例：增量/全量同步、冷热分离 |
| `runtime/delivery.Manager` | `runtime/delivery/manager.go` | 投递运行时：分片 Actor 模型、重试轮、Ack 索引 |
| `runtime/online.MemoryRegistry` | `runtime/online/registry.go` | 在线注册表：按 SessionID/UID/SlotID 索引连接 |
| `runtime/channelmeta` | `runtime/channelmeta/` | Channel runtime metadata resolver、合约、激活缓存、bootstrap/lease renewal、liveness/watcher 原语、channel leader repair/evaluate |
| `runtime/messageid` | `runtime/messageid/` | Snowflake 分布式 ID 生成 |
| `runtime/sequence` | `runtime/sequence/` | 序列号分配 |

## 2.1 Dependency Boundaries

- `internal/runtime/*` may depend on runtime contracts and `pkg/*`, but must not import `internal/access/*`, `internal/gateway/*`, `internal/usecase/*`, or `internal/app`.
- `internal/usecase/*` may depend on contracts, runtime primitives, and `pkg/*`, but must not import `internal/access/*` or `internal/app`.
- `internal/access/*` remains an adapter layer and converts transport DTOs into contract/usecase/runtime DTOs.
- Boundary guard: `GOWORK=off go test ./internal -run TestInternalImportBoundaries -count=1`.
...
| 类型 | 文件 | 说明 |
|------|------|------|
| `App` | app/app.go | 核心结构体，聚合 Cluster/ChannelLog/Gateway/所有 Usecase/Runtime |
| `Config` | app/config.go | 配置容器：Node, Storage, Cluster, API, Manager, Gateway, Conversation, Observability, Log |
| `Gateway` | gateway/gateway.go | 网关：管理 Listener/Session/Protocol 注册 |
| `Server` | gateway/core/server.go | 网关核心服务器：管理监听器、协议分发 |
| `Session` | gateway/session/session.go | 客户端会话：写队列、状态管理、Frame 读写 |
| `Handler` | access/gateway/handler.go | 网关桥接层：持有 Online/Messages/Presence 引用 |
| `Server` | access/api/server.go | HTTP API 服务器：消息发送、频道消息同步、Token、会话同步、健康检查 |
| `Client` | access/node/options.go | 节点间 RPC 客户端：投递/Presence/会话事实/channel leader repair |
| `Adapter` | access/node/options.go | 节点间 RPC 服务端适配器：含 channel leader repair/evaluate handler |
| `message.App` | usecase/message/app.go | 消息用例：持有 Cluster/Online/CommittedDispatcher/DeliveryAck |
| `delivery.App` | usecase/delivery/app.go | 投递用例：封装 delivery.Manager |
| `presence.App` | usecase/presence/app.go | 在线状态用例：持有 Online/Router/AuthorityClient/ActionDispatcher |
| `conversation.App` | usecase/conversation/app.go | 会话用例：持有 States/ChannelUpdate/Facts |
| `Manager` | runtime/delivery/manager.go | 投递管理器：分片 → Actor → Mailbox → RetryWheel |
| `shard` | runtime/delivery/shard.go | 投递分片：按 ChannelKey hash 分配 |
| `actor` | runtime/delivery/actor.go | 投递 Actor：单 Channel 的投递状态机 |
| `AckIndex` | runtime/delivery/ | Ack 索引：SessionID+MessageID → AckBinding 映射 |
| `MemoryRegistry` | runtime/online/registry.go | 在线注册表：按 SessionID/UID/SlotID 三维索引 |
...
入口: `app/app.go New` → `app/build.go build` → `app/lifecycle.go Start`

```
New(Config):
  → build(cfg):
    ① cfg.ApplyDefaultsAndValidate()
    ② 创建 build ResourceStack；任一构建步骤失败时按构建逆序关闭已注册资源，构建成功后 Release 给 App.Stop
    ③ 创建 Logger
    ...
    ⑨ 创建 ChannelMetaSync runtime resolver、runtime channel leader repairer/evaluator（app 只保留 wiring wrapper 与依赖装配）
    ⑩ 创建 Presence:
    ...
    ⑪ 创建 Delivery:
       ...
       → deliveryusecase.New
    ⑫ 创建 CommittedDispatcher: committedFanout(asyncCommittedDispatcher subscriber)
...
入口: `access/gateway/frame_router.go OnFrame`
...
message.App.Send(ctx, cmd):
  ...
  ⑨ sendDurable(ctx, cmd):
     ...
     b. sendWithEnsuredMeta: 带元数据刷新重试的 Append
     ...
     d. dispatcher.SubmitCommitted(ctx, messageevents.MessageCommitted):
        → committedFanout 为每个订阅者 clone 事件，并把订阅者错误返回给 message 用例记录日志
        → asyncCommittedDispatcher 作为当前订阅者异步分发已提交消息
...
入口: `runtime/delivery/manager.go Submit`
...
入口: `usecase/presence/gateway.go Activate`
...
入口: `usecase/conversation/sync.go Sync`
...
入口: `access/gateway/frame_router.go handleRecvAck`
...
入口: `access/api/channel_messagesync.go`

```
HTTP POST /channel/messagesync
  ↓
API Server 解析旧版兼容请求:
  login_uid / channel_id / channel_type / start_message_seq / end_message_seq / limit / pull_mode
  ↓
message.App.SyncChannelMessages(ctx, query):
  ① 校验 login_uid、channel_id、channel_type
  ② Person 频道通过 login_uid 规范化为 fake channel id
  ③ limit 默认 100、最大 10000
  ④ 通过 app 注入的 managerMessageReader 读取 Channel Leader 上的 committed log
     - 本地 Leader: channelhandler.LoadCommittedHW + channelhandler.SyncMessages
     - 远端 Leader: access/node/channel_messages_rpc.go 转发 sync_mode 查询
  ⑤ 返回旧版响应 {start_message_seq, end_message_seq, more, messages}
     messages 保持按 message_seq 升序输出；Person 频道响应 channel_id 会还原为当前登录用户看到的对端 UID
```
...
| `channel_messages` | → channel leader | access/node/channel_messages_rpc.go | 管理端消息查询与 `/channel/messagesync` 共用的 Channel Leader committed read RPC；`sync_mode=true` 时按旧版 start/end/pull_mode 范围语义返回升序消息 |
...
- **sendWithEnsuredMeta 只刷新一次权威结果，但可能顺带触发权威 leader repair**: 消息 Append 遇到 `ErrStaleMeta`、`ErrNotLeader` 或 `ErrRerouted` 时会触发一次刷新重试。若权威 `ChannelRuntimeMeta` 读到 `ErrNotFound`，刷新路径会先按 Slot 拓扑 bootstrap 缺失的运行时元数据；若读到的权威元数据 lease 已过期/即将过期，则当前 Channel Leader 所在节点会先尝试权威续租；若 lease 已经失效且本次刷新不在 leader 本机，刷新路径会把它当作 `leader_lease_expired` 交给当前 slot leader 重新选择/确认可写 leader，避免继续把写流量转发到一个已经失去租约却尚未被 controller 标 dead 的旧 leader。刷新后的权威 meta 若命中 `leader_missing` / `leader_not_replica` / `leader_dead` / `leader_draining` / `leader_lease_expired`，则会由当前 slot leader 选举并持久化新的 channel leader；持久化步骤只允许当前本地 slot leader 提案，若 repair 过程中 slot leadership 翻转，则会重新路由到新的 slot leader 重新执行，然后重新读取权威结果、应用到本地 ISR runtime，并把这份已应用视图返回给发送路径。重试后的本地 `Append` 若发现当前节点只是 Follower，或刷新后本地 runtime 已被移除但最新路由 meta 指向远端 leader，会按最新 meta 中的 Leader 走 node RPC 转发。
- **权威 runtime-meta reconcile ≠ replica reconcile**: `channelMetaSync` / `sendWithEnsuredMeta` 里的 reconcile 现在只负责两件事：当前 Channel Leader 的 lease 续租，以及在权威 leader 缺失/失效（包括 lease 已失效但 leader 尚未被 controller 判 dead）时由 slot leader 持久化新的 `ChannelRuntimeMeta.Leader` / `LeaderEpoch` / `LeaseUntilMS`；它不再把权威 `ChannelRuntimeMeta` 的 leader / replicas / ISR 跟着 Slot 拓扑漂移。`pkg/channel` 里的 replica reconcile probe 则负责在启动或 leader transfer 后重建 quorum-safe `CommitHW`，把副本从 `CommitReady=false` 拉到可服务状态。前者解决“路由是否仍然有效”，后者解决“现在是否安全可写”。
...
- **committedFanout + asyncCommittedDispatcher 的 preferLocal**: message 用例只发布 `messageevents.MessageCommitted`；`committedFanout` 负责进程内订阅者 fanout 和 clone，当前订阅者 `asyncCommittedDispatcher` 会把已提交消息直接进入本地 delivery runtime，并由 `distributedDeliveryPush` 按在线 route 把远端会话转成 node RPC push，不再先把整条已提交消息转发到 Channel Leader。这避免所有实时投递都绕经 Leader。
