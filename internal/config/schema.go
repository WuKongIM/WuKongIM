...
var schemaFields = []fieldSpec{
	...
	{TOMLPath: "backup.repository_id", EnvKey: "WK_BACKUP_REPOSITORY_ID", Kind: kindString, Group: "backup", Label: "Repository identity", Description: "Stable logical identity embedded in recovery points and shared by the independently verified primary and secondary repositories."},
	...
	{TOMLPath: "backup.target_generation", EnvKey: "WK_BACKUP_TARGET_GENERATION", Kind: kindString, Group: "backup", Label: "Restore target generation", Description: "Fresh successor-cluster generation required only in restore mode; it must differ from the selected recovery point source generation."},
	...
	{TOMLPath: "backup.signing_key_id", EnvKey: "WK_BACKUP_SIGNING_KEY_ID", Kind: kindString, Group: "backup", Label: "Manifest signing key", Description: "External asymmetric KMS key identifier used to sign canonical top-level recovery-point manifests."},
	{TOMLPath: "backup.trusted_signing_key_ids", EnvKey: "WK_BACKUP_TRUSTED_SIGNING_KEY_IDS", Kind: kindStringList, Group: "backup", Label: "Previous manifest signing keys", Description: "Retained previous asymmetric KMS key identifiers accepted only when verifying historical recovery points; the active signing key is always trusted automatically."},
	{TOMLPath: "backup.garbage_collector_role_arn", EnvKey: "WK_BACKUP_GARBAGE_COLLECTOR_ROLE_ARN", Kind: kindString, Group: "backup", Label: "Garbage collector role", Description: "AWS-compatible role assumed only by reference-safe garbage collection so upload credentials do not carry repository version-deletion authority."},
	...
	{TOMLPath: "backup.incremental_interval", EnvKey: "WK_BACKUP_INCREMENTAL_INTERVAL", Kind: kindDuration, Group: "backup", Label: "Incremental interval", Description: "Cadence for the Controller Leader to reevaluate incremental publication; it must be positive and no longer than the recovery-point interval."},
	{TOMLPath: "backup.restore_point_interval", EnvKey: "WK_BACKUP_RESTORE_POINT_INTERVAL", Kind: kindDuration, Group: "backup", Label: "Restore point interval", Description: "Cadence for publishing a fully verified cross-region recovery point; the default five minutes is the target RPO."},
	{TOMLPath: "backup.synthetic_full_interval", EnvKey: "WK_BACKUP_SYNTHETIC_FULL_INTERVAL", Kind: kindDuration, Group: "backup", Label: "Independent full interval", Description: "Target cadence for an independent full baseline; until object-reuse flattening is qualified the writer safely rematerializes source data."},
	{TOMLPath: "backup.chunk_size_bytes", EnvKey: "WK_BACKUP_CHUNK_SIZE_BYTES", Kind: kindUint64, Group: "backup", Label: "Chunk size bytes", Description: "Maximum plaintext bytes compressed and encrypted into one repository object; allowed range is 1 MiB through 256 MiB."},
	...
	{TOMLPath: "backup.max_parallel_partitions", EnvKey: "WK_BACKUP_MAX_PARALLEL_PARTITIONS", Kind: kindInt, Group: "backup", Label: "Parallel partitions", Description: "Maximum backup partitions captured concurrently on one node to bound CPU, disk, memory, and network pressure."},
	...
	{TOMLPath: "backup.monthly_retention_months", EnvKey: "WK_BACKUP_MONTHLY_RETENTION_MONTHS", Kind: kindInt, Group: "backup", Label: "Monthly retention", Description: "Number of monthly materialized-full recovery points to retain; zero disables the optional monthly tier."},
...
func supportedConfigKeysForBuilder() []string {
	return []string{
		...
		"WK_BACKUP_MONTHLY_RETENTION_MONTHS",
