...
func (s *StressAPI) add(c *wkhttp.Context) {
	...
	no, err := s.requestExchange(req.Addr, s.s.opts.External.APIUrl)
	if err != nil {
	...
	// 保存压测机数据
	nw := time.Now()
	err = s.s.store.AddOrUpdateTester(wkdb.Tester{
		No:        no,
		Addr:      req.Addr,
		CreatedAt: &nw,
		UpdatedAt: &nw,
...
func (s *StressAPI) remove(c *wkhttp.Context) {
	var req struct {
	...
	// 移除压测机数据
	err := s.s.store.RemoveTester(req.No)
	if err != nil {
...
func (s *StressAPI) infoList(c *wkhttp.Context) {
	...
	testers, err := s.s.store.GetTesters()
	if err != nil {
	...
	timeoutCtx, cancel := context.WithTimeout(s.s.ctx, 5*time.Second)
	defer cancel()
...
func (s *StressAPI) start(c *wkhttp.Context) {
	...
	tester, err := s.s.store.GetTester(req.No)
	if err != nil {
...
// 请求压测机系统信息
func (s *StressAPI) requestSystemInfo(addr string) (*testerInfoResp, error) {
...
// 请求压测机开始压测
func (s *StressAPI) requestStart(addr string, task *taskCfg) error {
...
// 请求压测机停止压测
func (s *StressAPI) requestStop(addr string) error {
...
// 请求压测机报告
func (s *StressAPI) requestReport(addr string) (*statsResp, error) {
