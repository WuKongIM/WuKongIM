...
func NewConversationManager(s *Logic) *ConversationManager {
...
func (cm *ConversationManager) calcLoop() {
	for {
		...
		message := messageMap["message"].(*model.Message)
...
func (cm *ConversationManager) PushMessage(message *model.Message, subscribers []string) {
	if !cm.s.opts.Conversation.On {
		return
	}
...
func (cm *ConversationManager) calConversation(message *model.Message, subscriber string) {
