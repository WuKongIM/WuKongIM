...
func NewWebhook(s *Logic) *Webhook {
...
// 通知离线消息
func (w *Webhook) notifyOfflineMsg(msg *model.Message, large bool, subscribers []string) {
	compress := ""
	toUIDs := subscribers
	...
	w.TriggerEvent(&Event{
		...
		Data: model.MessageOfflineNotify{
			MessageResp: model.MessageResp{
...
func (w *Webhook) notifyQueueLoop() {
	...
	if w.s.opts.WebhookOn() {
		for {
			...
			if len(messages) > 0 {
				messageResps := make([]*model.MessageResp, 0, len(messages))
				for _, msg := range messages {
					resp := &model.MessageResp{}
					resp.From(msg.(*model.Message), nil)
				...
				for _, message := range messages {
					messageID := message.(*model.Message).MessageID
