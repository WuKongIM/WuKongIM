...
func NewDeliveryManager(s *Logic) *DeliveryManager {
...
func (d *DeliveryManager) startDeliveryMessages(messages []*model.Message, large bool, syncOnceMessageSeqMap map[int64]uint32, subscribers []string, fromUID string, fromDeivceFlag wkproto.DeviceFlag, fromDeviceID string) {
...
func (d *DeliveryManager) deliveryMessages(messages []*model.Message, large bool, syncOnceMessageSeqMap map[int64]uint32, subscribers []string, fromUID string, fromDeivceFlag wkproto.DeviceFlag, fromDeviceID string) {
	...
	for _, subscriber := range subscribers {
		...
		for _, recvConn := range recvConns {
			...
			for _, m := range messages {
				...
				gatewayID := GetGatewayID(recvConn)
				cloneMsg.ToUID = subscriber
				cloneMsg.ToGatewayID = gatewayID
			...
			d.s.dataOut(recvConn, recvPackets...)
...
func (d *DeliveryManager) startRetryDeliveryMsg(msg *model.Message) {
	d.deliveryMsgPool.Submit(func() {
		d.retryDeliveryMsg(msg)
	})
}
func (d *DeliveryManager) retryDeliveryMsg(msg *model.Message) {
	...
	recvConn := d.getRecvConn(msg.ToGatewayID, msg.ToClientID)
	if recvConn == nil {
	...
	d.s.dataOut(recvConn, recvPacket)
}

// get recv
func (d *DeliveryManager) getRecvConns(subscriber string, fromUID string, fromDeivceFlag wkproto.DeviceFlag, fromDeviceID string) []gatewaycommon.Conn {
	toConns := d.s.gatewayManager.getConnsWithUID(subscriber)
	d.Debug("GetConnsWithUID", zap.String("subscriber", subscriber), zap.Int("conns", len(toConns)))
	conns := make([]gatewaycommon.Conn, 0, len(toConns))
	if len(toConns) > 0 {
...
func (d *DeliveryManager) getRecvConn(gatewayID string, connID int64) gatewaycommon.Conn {
	gatewaycli := d.s.gatewayManager.get(gatewayID)
	if gatewaycli == nil {
		return nil
	}
	return gatewaycli.GetConn(connID)
}

// 客户端是发送者自己
func (d *DeliveryManager) clientIsSelf(conn gatewaycommon.Conn, fromUID string, fromDeivceFlag wkproto.DeviceFlag, fromDeviceID string) bool {
