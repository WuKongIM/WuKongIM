...
func NewChannel(channelInfo *wkstore.ChannelInfo, s *Logic) *Channel {
...
func (c *Channel) Put(messages []*model.Message, customSubscribers []string, fromUID string, fromDeviceFlag wkproto.DeviceFlag, fromDeviceID string) error {
	//########## get subscribers ##########
...
func (c *Channel) storeMessageToUserQueueIfNeed(messages []*model.Message, subscribers []string) (map[int64]uint32, error) {
	if len(messages) == 0 {
		return nil, nil
	}
...
func (c *Channel) updateConversations(m *model.Message, subscribers []string) {
