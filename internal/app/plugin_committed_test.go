...
func TestNewBuildsPluginCommittedRouterWhenPluginEnabled(t *testing.T) {
	...
	router, ok := fanout.subscribers[1].(pluginCommittedRouter)
	require.Truef(t, ok, "second subscriber should be pluginCommittedRouter, got %T", fanout.subscribers[1])
	...
	require.Len(t, deliveryFanout.subscribers, 1)
