...
func managerControlMonitorMetricDefinitions() []monitorMetricDefinition {
	...
	return []monitorMetricDefinition{
		...
		monitorCatalogLabeledMetric(category, "controllerOldestTaskAge", incident, accessmanager.RealtimeMonitorToneCritical, "s", []string{"kind", "status", "step", "source"}, 8, monitorSummaryLatestMax, prometheusZeroFallback("max by (kind, status, step, source) (wukongim_controller_task_oldest_age_seconds{status!=\"completed\"})")),
...
func managerNodeMonitorMetricDefinitions() []monitorMetricDefinition {
	...
	return []monitorMetricDefinition{
		...
		// Scheduler coalescing and rescheduling are successful progress, not admission errors.
		// An observed admission rate proves zero; missing metrics must remain unavailable.
		monitorCatalogLabeledMetric(category, "runtimePoolAdmissionErrorRate", incident, accessmanager.RealtimeMonitorToneCritical, "events/s", []string{"component", "pool", "queue", "priority", "result"}, 8, monitorSummaryLatestSum, "sum by (component, pool, queue, priority, result) (rate(wukongim_runtime_pool_admission_total{result!=\"ok\"}[%s]) unless rate(wukongim_runtime_pool_admission_total{component=\"slot\",pool=\"scheduler\",queue=\"scheduler\",priority=\"none\",result=~\"coalesced|dirty|requeued\"}[%s])) or on() (0 * sum(rate(wukongim_runtime_pool_admission_total[%s])))"),
