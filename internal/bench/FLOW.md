...
- `chatlifecycle`: deterministic three-worker chat lifecycle planning, authenticated protocol-v2 worker control, coordinator-owned globally apportioned grant delivery, bounded verification/evidence, group setup, preflight, and service observation. Coordinator assignment, start, grant, status, and checkpoint barriers use fixed concurrent worker rounds; observation spans readiness and the measured run. Assignments disable worker-local primary token release, and one fail-closed sequenced grant vector drives all three workers every logical second without Docker or resumable state.
