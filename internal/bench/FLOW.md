...
- `chatlifecycle`: deterministic three-worker chat lifecycle planning, authenticated protocol-v2 worker control, coordinator-owned globally apportioned grant delivery, bounded verification/evidence, group setup, preflight, service observation, and an independent bounded natural hot-cold-reheat proof module. Coordinator assignment, start, grant, status, and checkpoint barriers use fixed concurrent worker rounds; observation spans readiness and the measured run. Assignments disable worker-local primary token release, and one fail-closed sequenced grant vector drives all three workers every logical second without Docker or resumable state. The lifecycle proof leases only current revisit timers, asynchronously probes three nodes without eviction, transiently merges every bounded batch before one atomic state transition, admits the existing scheduled real SEND through a fenced worker call no later than its deterministic due instant, proves sequence continuity from post-reheat probes, and reconciles low-cardinality metadata-create deltas against concurrent expected-unique growth. Its mapping boundary accepts a copied live 256-entry assignment, while current worker composition uses the validated continuous no-migration profile until a future coordinator passes the live preflight view; final verdict integration also remains owned by the later coordinator verdict phase.
