...
- `chatlifecycle`: deterministic three-worker chat lifecycle planning, authenticated protocol-v2 worker control, coordinator-owned globally apportioned grant delivery, bounded verification/evidence, group setup, preflight, and service observation. Coordinator assignments disable worker-local primary token release; one sequenced grant vector drives all three workers without Docker or resumable state.
