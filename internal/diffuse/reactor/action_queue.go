...
type actionQueue struct {
	...
	size          uint64
	left          []reactor.DiffuseAction // 左边队列
	right         []reactor.DiffuseAction // 右边队列, 左右的目的是为了重复利用内存
	nodrop        []reactor.DiffuseAction // 不能drop的消息
	mu            sync.Mutex
...
func newActionQueue(size uint64, ch bool,
	...
	q := &actionQueue{
		...
		left:          make([]reactor.DiffuseAction, size),
		right:         make([]reactor.DiffuseAction, size),
		nodrop:        make([]reactor.DiffuseAction, 0),
...
func (q *actionQueue) add(msg reactor.DiffuseAction) bool {
	q.mu.Lock()
	defer q.mu.Unlock()
	if q.idx >= q.size {
		return false
...
// 必须要添加的消息不接受drop
func (q *actionQueue) mustAdd(msg reactor.DiffuseAction) {
	q.mu.Lock()
	defer q.mu.Unlock()
	q.nodrop = append(q.nodrop, msg)
}

func (q *actionQueue) get() []reactor.DiffuseAction {
	q.mu.Lock()
	defer q.mu.Unlock()
	q.cycle++
	sz := q.idx
	...
	var result []reactor.DiffuseAction
	if len(q.nodrop) > 0 {
		ssm := q.nodrop
		q.nodrop = make([]reactor.DiffuseAction, 0)
		result = append(result, ssm...)
	}
	return append(result, t[:sz]...)
}

func (q *actionQueue) targetQueue() []reactor.DiffuseAction {
	var t []reactor.DiffuseAction
	if q.leftInWrite {
		t = q.left
	} else {
		t = q.right
...
func (q *actionQueue) tryAdd(msg reactor.DiffuseAction) bool {
	if !q.rl.Enabled() {
		return true
	}
	if q.rl.RateLimited() {
