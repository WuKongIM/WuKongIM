...
One lazily created owner per logical Slot Raft Group admits at most 256 queued unique identities, coalesces duplicates, collects arrivals behind an active proposal for at most 200 milliseconds, and pipelines at most two at-most-64-item proposals. The short collection window amortizes fixed Raft commit cost without delaying an idle Slot's first cold identity, while the two-batch pipeline prevents one future from serializing later cold identities without flooding a Slot leader with single-item proposals. Caller cancellation detaches only that waiter after submission, while Service close rejects new work, cancels queued waiters, and boundedly joins all node-owned proposals. The caller context owns the total queue wait: the two-second bound applies independently to each submitted proposal and authoritative reread, so a healthy waiter queued behind another bounded batch is not failed merely because their combined wall time exceeds one batch bound.
