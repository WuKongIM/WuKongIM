...
Node.AppendChannel / AppendChannelBatch
  ...
  -> Append: EnsureChannelMeta from append-only ChannelMetaEnsurer when available
      ...
      -> if missing: join the lazy owner for the current logical Slot Raft Group;
         duplicate channel identities share one logical create, and unique work
         waits in a 256-entry bounded queue behind one in-flight batch
      -> before submit: reroute the batch, canonical-sort at most 64 unique
         entries, and derive every initial replica/leader placement from one
         current data-node snapshot rather than a caller-captured candidate
      -> submit one command 59 batch and treat both authoritative created=true
         and created=false as completed create attempts
      -> reread the aligned authoritative rows from the current physical Slot
         leaders; after an uncertain proposal, retry only identities proved
         missing by that reread, for at most two bounded retries
      -> return the final authoritative ChannelRuntimeMeta; never substitute the origin candidate
...
runtime, business, subscriber, and membership metadata. It registers the centrally allocated
...
An unloaded Channel follower resolves runtime metadata through that dedicated
runtime-meta service; it must never reuse a Channel replication service ID.
...
Command 59 completion is observed by the routed Slot Raft Group leader's proposer through
...
`channels.Service` keeps a combined runtime interface because the public Channel runtime `Cluster` surface and replication `transport.Server` surface are separate. `StaticMetaSource` is available for tests and smoke runs. `SlotMetaSource` adapts authoritative `pkg/db/meta` `ChannelRuntimeMeta` records into Channel runtime metadata for production wiring, including `RouteGeneration` for complete append-cache versioning and the durable write-fence token/version/reason/deadline used to block new leader appends. The generation remains cache metadata and is not added to Channel replication RPC codecs or machine decisions. `ResolveChannelMeta` remains read-only; `EnsureChannelMeta` is the append-only path that may create the initial ChannelRuntimeMeta through the Slot-owned create-only metadata port before any Channel runtime append is attempted. Existing rows never propose creation. Command 59 has one bounded batch wire format; count=1 uses that same path.

One lazily created owner per logical Slot Raft Group admits at most 256 queued unique identities, coalesces duplicates, and pipelines at most two at-most-64-item proposals. A queued batch behind active work submits immediately once four cold identities are ready; a smaller burst waits at most 500 milliseconds. This target-or-deadline policy amortizes fixed Raft commit cost without delaying an idle Slot's first cold identity, while the two-batch pipeline prevents one future from serializing later cold identities without flooding a Slot leader with single-item proposals. Caller cancellation detaches only that waiter after submission, while Service close rejects new work, cancels queued waiters, and boundedly joins all node-owned proposals. The caller context owns the total queue wait: the two-second bound applies independently to each submitted proposal and authoritative reread, so a healthy waiter queued behind another bounded batch is not failed merely because their combined wall time exceeds one batch bound.

At submission the owner revalidates route/leader/term/config, canonical-sorts identities, and asks the placement resolver to derive the whole batch from one latest data-node snapshot. The proposer validates the batch before admission, and its authoritative completion observer records one closed result for every logical item rather than one result per Raft entry. An uncertain proposal is never blindly replayed: one authoritative aligned batch reread recovers present rows and only proved-missing identities may retry, at most twice while the caller remains active. Ordinary runtime-meta upsert remains separate for migration and repair. `SlotMetaSource` emits low-cardinality metadata resolve sub-stages for Slot meta read, initial placement/build, missing-meta write/propose, aggregate create/write, and final reread, plus bounded Slot-level queue/coalesced/batch-shape metrics, so cold activation tail latency can be attributed before pprof. In the default runtime, `meta_create_propose` wraps the Slot metadata creator call; `meta_create_propose_local` and `meta_create_propose_forward` split origin-side routing, `meta_create_slot_propose_submit` times local `Runtime.Propose`, and `meta_create_slot_propose_wait` times the subsequent Multi-Raft future wait. The default proposer also bridges the append stage observer into `pkg/slot/multiraft`, allowing the same Channel runtime stage histogram to report `meta_create_slot_control_wait`, `meta_create_slot_raft_commit_wait`, `meta_create_slot_fsm_apply`, `meta_create_slot_fsm_commit`, and `meta_create_slot_mark_applied`.

The per-Slot create owner is process-supervised. Its placement candidates carry
the exact control revision used to derive them; resolution waits within the
existing batch deadline for that exact revision, while a newer candidate
revision proves the submitted route stale and forces a reroute. This prevents
startup reconciliation from mixing route and health snapshots or exhausting
immediate retries inside their legitimate publication window. The
post-proposal reread follows current Slot authority rather than retaining the
submit-time leader fence, and only an authoritative `ErrNotFound` permits a
retry. Build, creator-call, and final-reread stage durations are recorded at
their physical batch boundaries; a pre-submit failure cannot emit a successful
proposal stage.
