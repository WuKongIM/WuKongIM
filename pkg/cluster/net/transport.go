...
// TransportServerConfig configures a transportv2-backed cluster RPC server.
...
// TransportServer serves typed cluster RPCs over pkg/transportv2.
...
func NewTransportServer(cfg TransportServerConfig) *TransportServer {
	...
	if err != nil {
		panic(fmt.Sprintf("cluster/net: create transportv2 server: %v", err))
	}
...
// TransportClientConfig configures a transportv2-backed cluster RPC client.
...
// TransportClient sends typed cluster RPCs over pkg/transportv2.
...
func NewTransportClient(cfg TransportClientConfig) *TransportClient {
	...
	if err != nil {
		panic(fmt.Sprintf("cluster/net: create transportv2 client: %v", err))
	}
...
func normalizeTransportLimits(limits transportv2.Limits, queueSize int) transportv2.Limits {
	...
	if limits.DialFailureCooldown < 0 || limits.WriteTimeout < 0 || limits.ReadIdleTimeout < 0 {
		panic("cluster/net: negative transportv2 timeout")
	}
