...
func (n *Node) registerDefaultSlotHandlers(runtime *multiraft.Runtime) {
	...
	n.transportServer.Register(clusternet.RPCBackupContinuousMessage, backupContinuousMessageRPCHandler{node: n})
	n.transportServer.Register(clusternet.RPCBackupContinuousSlot, backupContinuousSlotRPCHandler{node: n})
}
...
func decodeSlotRaftBatch(payload []byte) ([]multiraft.Envelope, error) {
	if !bytes.HasPrefix(payload, []byte(slotRaftBatchMagic)) {
		return nil, fmt.Errorf("cluster: invalid Slot Raft batch frame")
	}
	...
	if count > uint64(len(payload)-pos)/2 {
		return nil, fmt.Errorf("cluster: Slot Raft batch envelope count %d exceeds payload size", count)
	}
	...
	for i := uint64(0); i < count; i++ {
		...
		if messageLen > uint64(len(payload)-pos) {
			return nil, fmt.Errorf("cluster: truncated Slot Raft message payload")
		}
	...
	if pos != len(payload) {
		return nil, fmt.Errorf("cluster: trailing bytes in Slot Raft batch frame")
	}
	return out, nil
}
...
func readSlotRaftBatchUvarint(payload []byte, pos *int) (uint64, error) {
	if pos == nil || *pos > len(payload) {
		return 0, fmt.Errorf("cluster: invalid Slot Raft batch cursor")
	}
	...
	case n > 0:
		*pos += n
		return value, nil
	case n == 0:
		return 0, fmt.Errorf("cluster: truncated Slot Raft batch varint")
	default:
		return 0, fmt.Errorf("cluster: oversized Slot Raft batch varint")
	}
}
