...
func (s *Store) UpdateSessionUpdatedAt(slotId uint32, uids []string, channelId string, channelType uint8) error {

	cmd := NewCMD(CMDUpdateSessionUpdatedAt, EncodeCMDUpdateSessionUpdatedAt(uids, channelId, channelType))
	cmdData, err := cmd.Marshal()
	if err != nil {
		return err
	}
