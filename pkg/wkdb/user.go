...
// 用户是否设置了插件
func (wk *wukongDB) ExistUserPlugin(uid string) (bool, error) {
	...
	value, closer, err := db.Get(key.NewUserColumnKey(id, key.TableUser.Column.PluginNo))
	if closer != nil {
		defer closer.Close()
	}
	if err != nil {
...
func (wk *wukongDB) UpdateUserPluginNo(uid string, pluginNo string) error {
	...
	if pluginNo == "" {
		if err = batch.Delete(key.NewUserColumnKey(id, key.TableUser.Column.PluginNo), wk.noSync); err != nil {
			return err
		}
	} else {
		if err = batch.Set(key.NewUserColumnKey(id, key.TableUser.Column.PluginNo), []byte(pluginNo), wk.noSync); err != nil {
			return err
		}
	}
...
func (wk *wukongDB) GetUserPluginNo(uid string) (string, error) {
