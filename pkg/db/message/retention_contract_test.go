...
func TestCompatibilityTruncatePreservesAdoptedRetentionInvariant(t *testing.T) {
	engine := openCompatEngine(t)
	store := mustForChannel(t, engine, "compat-truncate-retention:1", channel.ChannelID{ID: "compat-truncate-retention", Type: 1})
	defer store.Close()
	if _, err := store.Append([]channel.Record{
		compatTestRecord(t, 801, "compat-truncate-retention", "c-1"),
		compatTestRecord(t, 802, "compat-truncate-retention", "c-2"),
		compatTestRecord(t, 803, "compat-truncate-retention", "c-3"),
	}); err != nil {
		t.Fatalf("Append(): %v", err)
	}
	if err := store.AdoptRetentionBoundary(context.Background(), 2, "committed"); err != nil {
	...
	if err := store.Truncate(2); err != nil {
	...
	if state.LocalRetentionThroughSeq != 2 || state.RetainedMaxSeq != 2 {
		t.Fatalf("retention state = %+v, want local and retained max 2", state)
	}
	if got := store.LEO(); got != 2 {
		t.Fatalf("LEO after truncate = %d, want 2", got)
	}
	if _, ok, err := store.GetMessageBySeq(3); err != nil || ok {
		t.Fatalf("GetMessageBySeq(3) = ok %v err %v, want removed", ok, err)
	}
	if err := store.Truncate(1); !errors.Is(err, channel.ErrCorruptState) {
		t.Fatalf("Truncate below adopted boundary error = %v, want corrupt state", err)
	}
	if err := store.Truncate(3); !errors.Is(err, channel.ErrCorruptState) {
		t.Fatalf("Truncate above LEO error = %v, want corrupt state", err)
	}
	if err := store.Truncate(2); err != nil {
		t.Fatalf("Truncate at current LEO no-op: %v", err)
	}
