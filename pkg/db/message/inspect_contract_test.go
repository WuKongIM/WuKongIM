...
func TestInspectChannelsPointLookupHonorsIdentityCursorAndCancellation(t *testing.T) {
	store := openTestMessageStore(t)
	defer store.close(t)
	ctx := context.Background()
	log := mustAcquireChannel(t, store.db, "point:3", ChannelID{ID: "point", Type: 3})
	defer log.Close()
	if _, err := log.Append(ctx, []Record{{ID: 701, Payload: []byte("seed")}}, AppendOptions{}); err != nil {
		t.Fatalf("Append(): %v", err)
	}

	result, err := InspectChannels(ctx, store.db, InspectMessageRequest{ChannelKey: "point:3", Limit: 1})
	if err != nil || len(result.Rows) != 1 || result.Rows[0]["channel_id"] != "point" || !result.Done || result.ScannedRows != 1 {
		t.Fatalf("point InspectChannels() = (%+v, %v)", result, err)
	}
	result, err = InspectChannels(ctx, store.db, InspectMessageRequest{
		ChannelKey: "point:3", AfterChannelKey: "point:3", Limit: 1,
	})
	if err != nil || len(result.Rows) != 0 || !result.Done || result.ScannedRows != 0 {
		t.Fatalf("cursor-filtered point lookup = (%+v, %v)", result, err)
	}
	result, err = InspectChannels(ctx, store.db, InspectMessageRequest{ChannelKey: "missing:3", Limit: 1})
	if err != nil || len(result.Rows) != 0 || !result.Done {
		t.Fatalf("missing point lookup = (%+v, %v)", result, err)
	}
	canceled, cancel := context.WithCancel(ctx)
	cancel()
	if _, err := InspectChannels(canceled, store.db, InspectMessageRequest{ChannelKey: "point:3"}); !errors.Is(err, context.Canceled) {
		t.Fatalf("canceled point lookup error = %v", err)
	}
}

func TestInspectChannelsPointLookupRejectsCorruptCatalogValue(t *testing.T) {
	store := openTestMessageStore(t)
	defer store.close(t)
	batch := store.engine.NewBatch()
	defer batch.Close()
	if err := batch.Set(encodeCatalogKey("corrupt-point:1"), []byte{0x01}); err != nil {
		t.Fatalf("Set(): %v", err)
	}
	if err := batch.Commit(true); err != nil {
		t.Fatalf("Commit(): %v", err)
	}
	if _, err := InspectChannels(context.Background(), store.db, InspectMessageRequest{ChannelKey: "corrupt-point:1"}); err == nil {
		t.Fatal("InspectChannels() error = nil, want corrupt catalog rejection")
	}
}
