...
func TestFSMReplacesRestoreCoordinationStateWithRevisionFence(t *testing.T) {
	...
	sm, _ := initializedStateMachine(t, 1)
	expected := uint64(1)
	restoreState := fsmVerifiedRestoreState()
	restoreState.Plan.Status = state.RestoreStatusPlanned
	restoreState.Plan.VerifiedAtUnixMillis = 0
	for hashSlot := range restoreState.Plan.Partitions {
		restoreState.Plan.Partitions[hashSlot] = state.RestorePartition{
			HashSlot: uint16(hashSlot),
			Status:   state.RestorePartitionPending,
		}
	}
...
func TestFSMSourceFenceIsOneWayAndOnlyConvergenceMayAdvanceIt(t *testing.T) {
	ctx := context.Background()
	sm, _ := initializedStateMachine(t, 1)
	...
	result, err := sm.Apply(ctx, 2, command.Command{
		Kind:             command.KindReplaceBackupCoordinationState,
		ExpectedRevision: &expected, Backup: &replacement,
	})
	require.NoError(t, err)
	require.False(t, result.Rejected)

	removed := replacement.Clone()
	removed.SourceFence = nil
	expected = 2
	result, err = sm.Apply(ctx, 3, command.Command{
		Kind:             command.KindReplaceBackupCoordinationState,
		ExpectedRevision: &expected, Backup: &removed,
	})
	require.NoError(t, err)
	require.True(t, result.Rejected)
	require.Equal(t, ReasonInvalidState, result.Reason)

	converged := replacement.Clone()
	converged.SourceFence.ConvergedAtUnixMillis = 1_800_000_001_000
	result, err = sm.Apply(ctx, 4, command.Command{
		Kind:             command.KindReplaceBackupCoordinationState,
		ExpectedRevision: &expected, Backup: &converged,
	})
	require.NoError(t, err)
	require.False(t, result.Rejected)

	replaced := converged.Clone()
	replaced.SourceFence.ID = "source-fence-2"
	expected = 3
	result, err = sm.Apply(ctx, 5, command.Command{
		Kind:             command.KindReplaceBackupCoordinationState,
		ExpectedRevision: &expected, Backup: &replaced,
	})
	require.NoError(t, err)
	require.True(t, result.Rejected)
	require.Equal(t, ReasonInvalidState, result.Reason)
	require.Equal(t, "source-fence-1",
		sm.Snapshot(ctx).Backup.SourceFence.ID)
}

func TestFSMActivationRequiresAuthorizedCleanupTransitionAndIsImmutable(
	t *testing.T,
) {
	ctx := context.Background()
	sm, _ := initializedStateMachine(t, 1)
	restore := fsmVerifiedRestoreState()
	expected := uint64(1)
	result, err := sm.Apply(ctx, 2, command.Command{
		Kind:             command.KindReplaceRestoreCoordinationState,
		ExpectedRevision: &expected, Restore: &restore,
	})
	require.NoError(t, err)
	require.False(t, result.Rejected)

	direct := restore.Clone()
	direct.Plan.Status = state.RestoreStatusActivated
	direct.Plan.ActivatedAtUnixMillis = 1_800_000_002_000
	direct.Plan.StagingCleanupCompletedAtUnixMillis = 1_800_000_002_000
	direct.Plan.Activation = fsmBreakGlassActivation(
		direct.Plan.ID, "audit-direct", 1_800_000_001_000,
	)
	expected = 2
	result, err = sm.Apply(ctx, 3, command.Command{
		Kind:             command.KindReplaceRestoreCoordinationState,
		ExpectedRevision: &expected, Restore: &direct,
	})
	require.NoError(t, err)
	require.True(t, result.Rejected)
	require.Equal(t, ReasonInvalidState, result.Reason)

	activating := restore.Clone()
	activating.Plan.Status = state.RestoreStatusActivating
	activating.Plan.UpdatedAtUnixMillis = 1_800_000_001_000
	activating.Plan.Activation = fsmBreakGlassActivation(
		activating.Plan.ID, "audit-1", 1_800_000_001_000,
	)
	result, err = sm.Apply(ctx, 4, command.Command{
		Kind:             command.KindReplaceRestoreCoordinationState,
		ExpectedRevision: &expected, Restore: &activating,
	})
	require.NoError(t, err)
	require.False(t, result.Rejected)

	replaced := activating.Clone()
	replaced.Plan.Activation = fsmBreakGlassActivation(
		replaced.Plan.ID, "audit-2", 1_800_000_001_000,
	)
	expected = 3
	result, err = sm.Apply(ctx, 5, command.Command{
		Kind:             command.KindReplaceRestoreCoordinationState,
		ExpectedRevision: &expected, Restore: &replaced,
	})
	require.NoError(t, err)
	require.True(t, result.Rejected)
	require.Equal(t, ReasonInvalidState, result.Reason)

	activated := activating.Clone()
	activated.Plan.Status = state.RestoreStatusActivated
	activated.Plan.UpdatedAtUnixMillis = 1_800_000_002_000
	activated.Plan.StagingCleanupCompletedAtUnixMillis = 1_800_000_002_000
	activated.Plan.ActivatedAtUnixMillis = 1_800_000_002_000
	result, err = sm.Apply(ctx, 6, command.Command{
		Kind:             command.KindReplaceRestoreCoordinationState,
		ExpectedRevision: &expected, Restore: &activated,
	})
	require.NoError(t, err)
	require.False(t, result.Rejected)

	expected = 4
	result, err = sm.Apply(ctx, 7, command.Command{
		Kind:             command.KindReplaceRestoreCoordinationState,
		ExpectedRevision: &expected, Restore: &restore,
	})
	require.NoError(t, err)
	require.True(t, result.Rejected)
	require.Equal(t, ReasonInvalidState, result.Reason)
	require.Equal(t, state.RestoreStatusActivated,
		sm.Snapshot(ctx).Restore.Plan.Status)
}

func fsmVerifiedRestoreState() state.RestoreCoordinationState {
	...
	partitions := make([]state.RestorePartition, hashSlotCount)
	for index := range partitions {
		partitions[index] = state.RestorePartition{
			HashSlot: uint16(index), Status: state.RestorePartitionConverged,
			TargetSlotID: uint32(index) + 1,
			LeaderNodeID: 1, LeaderTerm: 9, ConfigEpoch: 4,
			...
			MessageMerkleSHA256: strings.Repeat("f", 64),
			ReplicaCount:        3, ConvergedReplicas: 3,
			StartedAtUnixMillis:   1_753_400_203_000,
			InstalledAtUnixMillis: 1_753_400_204_000,
		}
	}
	return state.RestoreCoordinationState{Plan: &state.RestorePlan{
		...
		Status:                          state.RestoreStatusVerified,
		CreatedAtUnixMillis:             1_753_400_202_000,
		UpdatedAtUnixMillis:             1_753_400_205_000,
		VerifiedAtUnixMillis:            1_753_400_205_000,
		Partitions:                      partitions,
	}}
}

func fsmBreakGlassActivation(
	planID, auditID string,
	recordedAtUnixMillis int64,
) *backupartifact.RestoreActivationEvidence {
	audit := backupartifact.BreakGlassActivationAudit{
		ID: auditID, RestorePlanID: planID,
		Operator:               "recovery-admin",
		Reason:                 "All source Controller disks are permanently unavailable.",
		AuthorizedAtUnixMillis: recordedAtUnixMillis,
	}
