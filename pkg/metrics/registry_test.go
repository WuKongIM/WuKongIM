...
func TestControllerMetricsTrackAuditDerivedOldestTaskAge(t *testing.T) {
	...
	reg.Controller.taskOldestAge.now = func() time.Time { return now }
...
func TestControllerMetricsOldestTaskAgeAdvancesBetweenGathers(t *testing.T) {
	reg := New(1, "node-1")
	now := time.Unix(1000, 0)
	reg.Controller.taskOldestAge.now = func() time.Time { return now }
	...
	require.NoError(t, err)
	age := requireMetricFamily(t, families, "wukongim_controller_task_oldest_age_seconds")
	first := findMetricByLabels(t, age, map[string]string{
		"kind":   "slot_replica_move",
		"status": "running",
		"step":   "remove_voter",
		"source": "audit",
	...
	require.NoError(t, err)
	age = requireMetricFamily(t, families, "wukongim_controller_task_oldest_age_seconds")
	second := findMetricByLabels(t, age, map[string]string{
		"kind":   "slot_replica_move",
		"status": "running",
		"step":   "remove_voter",
		"source": "audit",
	...
	require.Equal(t, float64(52), second)
}
...
func TestChannelRuntimeMetricsTrackReactorAndWorkerRuntime(t *testing.T) {
	...
	reg.ChannelRuntime.ObserveAppendLatency("local", 7*time.Millisecond)
	reg.ChannelRuntime.ObserveAppendStage("meta_apply", "ok", 5*time.Millisecond)
	reg.ChannelRuntime.ObserveAppendWaitStage("store_append_wait", "quorum", "ok", 17*time.Millisecond)
	reg.ChannelRuntime.ObserveReplicationStage("follower_pull_rpc", "ok", 19*time.Millisecond)
	...
	reg.ChannelRuntime.ObservePendingMeta("released", "timeout")
	reg.ChannelRuntime.ObserveNeedMetaPull("retry", "other")
	reg.ChannelRuntime.ObserveMetaCache("hit")
	reg.ChannelRuntime.ObserveMetaCache("invalidate")
	reg.ChannelRuntime.SetISRAnomalyChannels(map[string]int{
		"isr_insufficient":        3,
		"no_leader":               1,
	...
	require.Len(t, workerDuration.GetMetric(), 3)
	...
	workerErrors := requireMetricFamily(t, families, "wukongim_channelv2_worker_task_error_total")
	require.Len(t, workerErrors.GetMetric(), 1)
	requireMetricLabels(t, workerErrors.GetMetric()[0], map[string]string{
		"node_id":   "8",
		"node_name": "node-8",
		"kind":      "rpc_pull",
		"error":     "timeout",
	...
	require.Len(t, rpcPullTotal.GetMetric(), 2)
	findMetricByLabels(t, rpcPullTotal, map[string]string{
		"node_id":   "8",
		"node_name": "node-8",
		"result":    "ok",
	})
	rpcPullErr := findMetricByLabels(t, rpcPullTotal, map[string]string{
		"node_id":   "8",
		"node_name": "node-8",
		"result":    "err",
	})
	require.Equal(t, float64(1), rpcPullErr.GetCounter().GetValue())
