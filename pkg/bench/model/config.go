...
type Scenario struct {
	...
	// Objectives declares the reviewed load targets that make a scenario comparable.
	Objectives ObjectivesConfig `json:"objectives" yaml:"objectives"`
...
// ObjectivesConfig declares the externally observable targets for a reviewed scenario.
type ObjectivesConfig struct {
	// Scale is the stable public scale name, such as small, medium, or large.
	Scale string `json:"scale" yaml:"scale"`
	// Standard marks a profile eligible for a standard stability verdict.
	Standard bool `json:"standard" yaml:"standard"`
	// IngressQPS is the total SEND rate expected during the measured run phase.
	IngressQPS Rate `json:"ingress_qps" yaml:"ingress_qps"`
	// OnlineFanoutQPS is the target successful delivery rate to online connections.
	OnlineFanoutQPS Rate `json:"online_fanout_qps" yaml:"online_fanout_qps"`
	// ToleranceRatio is the allowed relative deviation around ingress and fanout targets.
	ToleranceRatio float64 `json:"tolerance_ratio" yaml:"tolerance_ratio"`
	// ActiveChannelWindow is the maximum window in which every declared channel must receive traffic.
	ActiveChannelWindow time.Duration `json:"active_channel_window" yaml:"active_channel_window"`
	// RequireAllChannelsActive requires every declared channel to participate in measured traffic.
	RequireAllChannelsActive bool `json:"require_all_channels_active" yaml:"require_all_channels_active"`
}
...
type IdentityConfig struct {
	// TotalUsers is the full generated user pool, including online and offline users.
	// Zero preserves the v1 compatibility behavior of using online.total_users.
...
type OnlineConfig struct {
	...
	// Churn controls scheduled connection replacement during the measured phase.
...
// ChurnConfig describes deterministic online connection churn during a measured run.
type ChurnConfig struct {
	// Enabled activates scheduled churn windows.
	Enabled bool `json:"enabled" yaml:"enabled"`
	// Interval is the time between churn operations.
	Interval time.Duration `json:"interval" yaml:"interval"`
	// Ratio is the fraction of worker-local online identities churned per interval.
	Ratio float64 `json:"ratio" yaml:"ratio"`
	// SameUserRatio is the fraction of churn operations that reconnect the same UID.
	SameUserRatio float64 `json:"same_user_ratio" yaml:"same_user_ratio"`
	// IdentitySwapRatio is the fraction of churn operations that replace a UID with an offline identity.
	IdentitySwapRatio float64 `json:"identity_swap_ratio" yaml:"identity_swap_ratio"`
	// HistorySync must remain false for stability workloads that measure live traffic only.
	HistorySync bool `json:"history_sync" yaml:"history_sync"`
}
...
type ShardConfig struct {
	...
	Mode string `json:"mode" yaml:"mode"`
	// HashSlotSpread places one generated channel in each configured physical hash slot.
	HashSlotSpread bool `json:"hash_slot_spread" yaml:"hash_slot_spread"`
	// HashSlotCount is the physical hash-slot count used by HashSlotSpread.
	HashSlotCount uint16 `json:"hash_slot_count" yaml:"hash_slot_count"`
}
