		...
		for _, workers := range []int{1, 16} {
			for _, budgets := range []bool{false, true} {
				b.Run(fmt.Sprintf("Bytes%d/Workers%d/Budgets%t", size, workers, budgets), func(b *testing.B) {
					benchmarkTransportBudget(b, size, workers, 16, budgets)
				})
			}
		}
	}
...
// BenchmarkTransportSharedConnection keeps sixteen concurrent callers while
// varying only the number of active, warmed connection slots. This exposes
// queued work and executor handoff costs hidden by one caller per connection.
// Both endpoints share this process; use separate-process probes for acceptance.
func BenchmarkTransportSharedConnection(b *testing.B) {
	for _, size := range []int{64, 65537} {
		for _, activeConnections := range []int{1, 16} {
			for _, budgets := range []bool{false, true} {
				b.Run(fmt.Sprintf("Bytes%d/Connections%d/Budgets%t", size, activeConnections, budgets), func(b *testing.B) {
					benchmarkTransportBudget(b, size, 16, activeConnections, budgets)
				})
			}
		}
	}
...
// benchmarkTransportBudget excludes connection warmup and capability negotiation
// from timing. A fixed shard-to-slot mapping controls actual connection reuse.
func benchmarkTransportBudget(b *testing.B, size, workers, activeConnections int, budgets bool) {
