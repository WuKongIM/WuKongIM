# pkg/legacy/cluster 流程文档

## 1. 职责定位

旧分布式集群编排层，仅供 legacy `internal` 路径与过渡期工具编译期保留。负责旧 Multi-Raft Slot 的生命周期管理、请求路由与 Leader 转发、旧 Controller 协调（选举 / 任务分配 / 调和）、Hash Slot 迁移调度。
