# pkg/legacy/channel Flow

## 1. 职责定位

Legacy ISR 复制状态机频道模型。负责旧架构消息的追加、获取、多副本复制、持久化存储和故障恢复。
...
`channelplane` 负责 durable send 的寻址与重路由；`pkg/legacy/channel` 只在本地 leader 上执行 append、校验 epoch / fence / 归属并返回结构化结果。
