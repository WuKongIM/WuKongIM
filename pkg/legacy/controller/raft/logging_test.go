...
func TestNewEtcdRaftLoggerAddsControllerScope(t *testing.T) {
	base := newRecordingLogger("cluster.controller")

	logger := newEtcdRaftLogger(base, 9)
	logger.Infof("node %d elected", 9)

	entries := base.entries()
	require.Len(t, entries, 1)
	entry := entries[0]
	require.Equal(t, "INFO", entry.level)
	require.Equal(t, "cluster.controller.raft", entry.module)
	require.Equal(t, "node 9 elected", entry.msg)

	field, ok := entry.field("raftScope")
	require.True(t, ok)
	require.Equal(t, "controller", field.Value)

	field, ok = entry.field("nodeID")
	require.True(t, ok)
	require.Equal(t, uint64(9), field.Value)
}
