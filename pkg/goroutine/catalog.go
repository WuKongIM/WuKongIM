...
var defaultTaskCatalog = []TaskSpec{
	...
	{ID: TaskTransportRPCService, Module: ModuleTransport, Name: "rpc_service", Kind: TaskKindDynamic, PanicPolicy: PanicPolicyRepanic},
	...
	{ID: TaskSlotRaftWorker, Module: ModuleSlot, Name: "raft_worker", Kind: TaskKindDynamic, PanicPolicy: PanicPolicyRepanic},
...
func buildCatalog(specs []TaskSpec) (map[TaskID]TaskSpec, error) {
	...
	for _, spec := range specs {
		...
		if spec.Kind == TaskKindFixed && spec.Expected <= 0 {
			return nil, fmt.Errorf("goroutine: fixed task %q must declare a positive expected count", spec.ID)
		}
