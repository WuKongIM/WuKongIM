...
func TestGatewayProxyProtocolAddressesBeforeAuthenticationTCPAndWebSocket(t *testing.T) {
	...
	for _, trust := range []struct {
		...
		{"omitted", nil}, {"empty", []string{}}, {"restricted", []string{"127.0.0.1/32"}},
	} {
		for _, network := range []string{"tcp", "websocket"} {
			t.Run(trust.name+"/"+network, func(t *testing.T) {
				...
				gw, err := gateway.New(gateway.Options{
					...
					Listeners: []gateway.ListenerOptions{{Name: "client", Network: network, Address: "127.0.0.1:0", Transport: "gnet", Protocol: protocol, ProxyProtocolTrustedCIDRs: trust.cidrs}},
				})
				if err != nil {
					t.Fatal(err)
				}
