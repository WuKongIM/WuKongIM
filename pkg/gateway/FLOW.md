...
`pkg/gateway` 是可复用的客户端接入基础设施。它负责监听器绑定、底层传输适配、协议编解码、会话生命周期、认证握手、帧分发、transport 异步写、空闲关闭，以及网关级观测事件。
...
- `pkg/gateway` 可以依赖 `pkg/protocol/*`、`pkg/observability/sendtrace`、`pkg/wklog` 等通用包。
- `pkg/gateway` 不应依赖 `internal/access/*`、`internal/usecase/*`、`internal/runtime/*`、`internal/app`。
