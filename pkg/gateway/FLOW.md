...
**不负责**:
- 消息发送、RecvAck、Ping、presence activation 等业务路由 → `internal/access/gateway` 或 `internalv2/access/gateway`。
...
入口: `internal/app/build.go` 或 `internalv2/app` → `pkg/gateway.New` → `core.NewServer` → `Gateway.Start`

```text
app build:
  ① 创建 `internal/access/gateway` 或 `internalv2/access/gateway`.Handler，注入 message/presence usecase
...
- `internal/access/gateway` 和 `internalv2/access/gateway` 可以依赖 `pkg/gateway`，负责把 session lifecycle / `frame.Frame` 转为 message/presence 用例命令。
