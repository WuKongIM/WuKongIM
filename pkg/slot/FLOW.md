...
- **ChannelRuntimeMeta 写入单调保护**: `UpsertChannelRuntimeMeta` 会拒绝更旧的 `ChannelEpoch` / `LeaderEpoch`，同一 epoch 下也不会切换到不同 leader 或缩短 leader lease；repair / bootstrap 必须通过更高 epoch 或更长 lease 表达有效更新。
