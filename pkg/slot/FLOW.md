...
- **Channel 迁移不可逆边界**: drain 后续租 fence 会提升 `WriteFenceVersion` 并清空旧 `Cutover*` / `Drained*` proof，后续 cutover 必须重新 drain；abort 只允许在 leader commit / promote 之前的可回滚阶段，不能把已提交 leader transfer 或已 promote 的任务标成 aborted。
