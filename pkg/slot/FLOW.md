...
| 8 | CMDConversationState | (uid, channel_type, channel_id) | idx_cmd_conversation_active |

## 7. FSM 命令类型（16 种 + 2 个保留 ID）
...
| `cmdConversationStateRPCServiceID` | CMD 会话状态查询、upsert 与 read cursor 推进 | proxy/cmd_conversation_state_rpc.go |
...
- **CMDConversationState 是独立表**: CMD 离线同步只读写 Table ID 8，不能混用 `UserConversationState`；active index 仍按 UID owner 有界扫描。
...
- **RPC Handler 注册**: `proxy.New` 在构造时调用 `cluster.RPCMux().Handle(...)` 注册 6 个 handler，漏任一个该类远端查询会全部失败。
