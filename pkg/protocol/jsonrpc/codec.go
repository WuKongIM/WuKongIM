...
func ToFrame(packet interface{}) (frame.Frame, string, error) {
...
func FromFrame(reqId string, f frame.Frame) (interface{}, error) {
	...
	case frame.RECV:
		...
		return result, nil
	case frame.EVENT:
		event := f.(*frame.EventPacket)
		result := FromProtoEventNotification(event)
		return result, nil
	case frame.DISCONNECT:
		disconnect := f.(*frame.DisconnectPacket)
