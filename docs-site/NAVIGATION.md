...
- **WuKongEasySDK / WuKongEasySDK** `/{lang}/sdk/easy` — 当前 Product Gateway 不支持 EasySDK 使用的 JSON-RPC CONNECT；修复并完成端到端验证后再发布。 / The Product Gateway does not currently support EasySDK JSON-RPC CONNECT; publish only after a runtime fix and end-to-end verification.
  - **5 分钟集成 iOS / 5-minute iOS integration** `/{lang}/sdk/easy/ios/getting-started` — JSON-RPC CONNECT 尚未受支持；保留 v1.0.3 源码评估，等待运行时验证。 / JSON-RPC CONNECT is unsupported; retain the v1.0.3 source review until runtime verification exists.
  - **5 分钟集成 Android / 5-minute Android integration** `/{lang}/sdk/easy/android/getting-started` — JSON-RPC CONNECT 尚未受支持；保留 v1.0.3 源码评估，等待运行时验证。 / JSON-RPC CONNECT is unsupported; retain the v1.0.3 source review until runtime verification exists.
  - **5 分钟集成 Flutter / 5-minute Flutter integration** `/{lang}/sdk/easy/flutter/getting-started` — JSON-RPC CONNECT 尚未受支持；保留 v1.0.4 源码评估，等待运行时验证。 / JSON-RPC CONNECT is unsupported; retain the v1.0.4 source review until runtime verification exists.
  - **5 分钟集成 Web / 5-minute Web integration** `/{lang}/sdk/easy/javascript/getting-started` — JSON-RPC CONNECT 尚未受支持；保留 v2.0.2 源码评估，等待运行时验证。 / JSON-RPC CONNECT is unsupported; retain the v2.0.2 source review until runtime verification exists.
