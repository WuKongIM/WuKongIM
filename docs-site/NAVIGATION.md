...
- **产品 HTTP API（Beta 子集） / Product HTTP API (Beta subsets)** `/{lang}/api/product-http` — 浏览当前 Product HTTP Beta 接口。 / Browse the current Product HTTP Beta APIs.
  - **用户 / Users** `/{lang}/api/product-http/users` — 开发身份的设备 Token 元数据。 / Device-token metadata for development identities.
    - **登记快速开始用户的开发 Token / Register the quickstart user's development token** **POST** `/{lang}/api/product-http/users/setQuickstartUserToken` — 保存开发身份的设备 Token 元数据；它不会启用生产 CONNECT 鉴权。 / Stores device-token metadata for a development identity. It does not enable production CONNECT authentication.
  - **路由发现 / Route Discovery** `/{lang}/api/product-http/routing` — 客户端 Gateway 接入地址。 / Client Gateway ingress addresses.
  ...
  - **Channel / Channels** `/{lang}/api/product-http/channels` — Channel 元数据、订阅者与允许或拒绝名单。 / Channel metadata, subscribers, and allow or deny lists.
    ...
    - **移除 Channel 的全部持久订阅者 / Remove all durable Channel subscribers** **POST** `/{lang}/api/product-http/channels/removeAllChannelSubscribers` — 移除全部持久订阅者，但不删除或解散 Channel；个人 Channel 会被拒绝。 / Removes all durable subscribers without deleting or disbanding the Channel. Person Channels are rejected.
    - **替换临时 Channel 订阅者 / Replace temporary Channel subscribers** **POST** `/{lang}/api/product-http/channels/setTemporaryChannelSubscribers` — 替换临时 Channel（Type 8）的全部成员；临时与持久订阅者相互独立。 / Replaces all members of the temporary Channel (Type 8). Temporary and durable subscribers are separate.
    - **添加 Channel 黑名单成员 / Add Channel denylist members** **POST** `/{lang}/api/product-http/channels/addChannelDenylistMembers` — 向 Channel 拒绝列表添加 UID；路由沿用 blacklist 命名。 / Adds UIDs to the Channel denylist; the route retains its legacy blacklist name.
    - **移除 Channel 黑名单成员 / Remove Channel denylist members** **POST** `/{lang}/api/product-http/channels/removeChannelDenylistMembers` — 从 Channel 范围的拒绝列表中移除给定 UID。 / Removes the listed UIDs from the Channel-scoped denylist.
    - **移除 Channel 的全部黑名单成员 / Remove all Channel denylist members** **POST** `/{lang}/api/product-http/channels/removeAllChannelDenylistMembers` — 通过内部有界遍历清空 Channel 范围的拒绝列表。 / Clears the Channel-scoped denylist through bounded internal traversal.
    - **添加 Channel 白名单成员 / Add Channel allowlist members** **POST** `/{lang}/api/product-http/channels/addChannelAllowlistMembers` — 向 Channel 允许列表添加非空白 UID；路由沿用 whitelist 命名。 / Adds non-blank UIDs to the Channel allowlist; the route retains its legacy whitelist name.
    - **移除 Channel 白名单成员 / Remove Channel allowlist members** **POST** `/{lang}/api/product-http/channels/removeChannelAllowlistMembers` — 从 Channel 范围的允许列表中移除给定非空 UID。 / Removes the listed non-empty UIDs from the Channel-scoped allowlist.
    - **移除 Channel 的全部白名单成员 / Remove all Channel allowlist members** **POST** `/{lang}/api/product-http/channels/removeAllChannelAllowlistMembers` — 通过内部有界遍历清空 Channel 范围的允许列表。 / Clears the Channel-scoped allowlist through bounded internal traversal.
  - **会话 / Conversations** `/{lang}/api/product-http/conversations` — 会话同步、未读、隐藏与激活状态。 / Conversation sync, unread, hide, and activation state.
    - **同步一页会话 / Synchronize a Conversation page** **POST** `/{lang}/api/product-http/conversations/listConversations` — 返回一页有界会话。原样回传 next_cursor 直到 done=true；空页不表示完成。 / Returns one bounded Conversation page. Reuse next_cursor unchanged until done=true; an empty page is not completion.
    - **重试未解析的会话键 / Retry unresolved Conversation keys** **POST** `/{lang}/api/product-http/conversations/retryConversations` — 重试最多 200 个未解析键，不回退已完成的列表 coverage；重复键会合并。 / Retries up to 200 unresolved keys without rewinding completed list coverage; duplicate keys are merged.
    - **清除会话未读 / Clear Conversation unread** **POST** `/{lang}/api/product-http/conversations/clearConversationUnread` — 把 read_seq 推进到最新已提交 Channel head；合同不发布旧 message_seq，运行时会忽略它。 / Advances read_seq to the latest committed Channel head. The contract omits legacy message_seq; the runtime ignores it.
    - **设置会话未读上限 / Set maximum Conversation unread** **POST** `/{lang}/api/product-http/conversations/setConversationUnread` — 推进 read_seq，使未读数不超过 unread；该操作不会增加未读数。 / Advances read_seq so no more than unread messages remain. It never increases the unread count.
    - **隐藏会话至当前 head / Hide Conversation through the current head** **POST** `/{lang}/api/product-http/conversations/hideConversation` — 把会话隐藏到当前已提交 head；不会删除 Channel 或成员关系。 / Hides this Conversation through the current committed head. It does not delete the Channel or membership.
    - **激活会话 / Activate Conversation** **POST** `/{lang}/api/product-http/conversations/activateConversation` — 记录显式打开、切换或恢复动作，用于会话排序；消息路径不会隐式激活。 / Records an explicit open, switch, or restore action for Conversation ordering; message paths do not activate it.
