...
describe('Product HTTP management OpenAPI integration', () => {
  ...
  test('generates bilingual one-operation Fumadocs pages grouped by tag', async () => {
    for (const group of productHTTPManagementOpenAPIGroups) {
      for (const suffix of ['.mdx', '.en.mdx']) {
        ...
        for (const deferral of group.deferrals?.items ?? []) {
          for (const route of deferral.routes) expect(index).toContain(`\`${route}\``);
          expect(index).toContain(deferral.reason[locale]);
        }
  ...
  test('publishes the separate downloadable management contract and deferral notes', async () => {
    const [route, specification, channels, channelsEn] = await Promise.all([
      ...
      source('../content/docs/api/product-http/channels/index.en.mdx'),
    ]);

    expect(route).toContain('product-http-management.openapi.json');
    for (const path of deferredOperations) expect(specification).toContain(path);
    for (const fact of [
      '缺少字段校验',
      'Key 校验较弱',
      'channel_type 为 0 时的行为',
      '全量替换的输入校验较弱',
      '查询参数未校验',
      '返回无界完整列表',
    ]) {
      expect(channels).toContain(fact);
    }
    for (const fact of [
      'lacks field validation',
      'weak key validation',
      'Type-zero behavior',
      'Full-replacement input validation is weak',
      'query is unvalidated',
      'unbounded full list',
    ]) {
      expect(channelsEn).toContain(fact);
    }
  });
});
