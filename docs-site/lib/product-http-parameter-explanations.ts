  ...
  schemas: {
    ...
    SendMessageRequest: {
      ...
      properties: {
        ...
        client_msg_no: explanation('可选的客户端幂等键；省略或空白时生成唯一编号并在响应返回。重试同一消息时应显式复用编号。', 'Optional client idempotency key; omitted or blank values generate a unique number returned in the response. Reuse an explicit key when retrying the same message.'),
    ...
    SendMessageResponse: {
      ...
      properties: {
        client_msg_no: explanation('实际提交的客户端消息编号；未提供时由服务端生成。', 'Client message number submitted with this send; generated by the server when omitted.'),
    ...
    LegacyMessage: {
      ...
      properties: {
        ...
        client_msg_no: explanation("原客户端编号；旧记录为空且有消息 ID 时，返回稳定的 wk3-legacy-<message_id> 只读兼容编号。可用于 /messages 查询，不是发送幂等键或事件写入键；原始记录不改写。", "Original client number, or the stable read-only alias wk3-legacy-<message_id> for an old empty-number record with a message ID. The alias supports /messages lookup; it is not a SEND idempotency or event mutation key. Stored records are unchanged."),
    ...
    ConversationSyncLegacyItem: {
      ...
      properties: {
        ...
        last_client_msg_no: explanation('末条消息的客户端编号；空编号旧记录与 recents 使用同一只读兼容编号。', 'Last-message client number; old empty-number records use the same read-only alias as recents.'),
        ...
        recents: explanation('按 msg_count 返回最近消息；为修复旧客户端空编号冲突，最新空编号消息或修改过的末条消息可在游标已前移时再次返回，已读和删除位置不变。', 'Recent messages bounded by msg_count. To repair old empty-key collisions, the latest empty-number message or edited tail may be returned again after the client cursor advances; read and delete positions are unchanged.'),
      },
    },
...
export function applyProductHTTPParameterExplanations(
  ...
  for (const [name, rawText] of Object.entries(productHTTPParameterExplanations.schemas)) {
    ...
    if (!schema) continue;
    const projection = name === 'LegacyMessage' ? components.schemas?.UpdatedMessage : undefined;
    if (projection) projection.description = text.description[locale];
    ...
    for (const [propertyName, propertyText] of Object.entries(text.properties ?? {})) {
      ...
      const projected = projection?.properties?.[propertyName];
      if (projected) projected.description = propertyText[locale];
    }
  }
}
