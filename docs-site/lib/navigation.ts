...
function publishedEasySDKGroup(): NavigationGroup {
  return publishedGroup(
    ...
    'WuKongEasySDK',
    'WuKongEasySDK',
    '选择 iOS、Android、Flutter 或 Web 快速接入，用固定版本完成 Alice/Bob 在线双向消息；核心路径已有服务端线协议凭据，平台构建、设备运行和生产 Token 校验仍需单独验收。',
    'Choose an iOS, Android, Flutter, or Web quickstart and use a pinned release for Alice/Bob online messaging; the core path has a server-side wire receipt, while platform builds, device runs, and production token verification still require separate acceptance.',
    [
      publishedPage(
        'ios/getting-started',
        'iOS 快速接入',
        'iOS quickstart',
        '精确安装 v1.1.0，用已支持的 JSON-RPC CONNECT 完成单聊收发、监听清理和 Alice/Bob 验收。',
        'Install exactly v1.1.0 and use supported JSON-RPC CONNECT for person messaging, listener cleanup, and Alice/Bob acceptance.',
      ),
      publishedPage(
        'android/getting-started',
        'Android 快速接入',
        'Android quickstart',
        '精确安装 v1.0.4，用已支持的 JSON-RPC CONNECT 处理单例、单聊收发、清理和 Alice/Bob 验收。',
        'Install exactly v1.0.4 and use supported JSON-RPC CONNECT for singleton ownership, person messaging, cleanup, and Alice/Bob acceptance.',
      ),
      publishedPage(
        'flutter/getting-started',
        'Flutter 快速接入',
        'Flutter quickstart',
        '精确安装 v1.1.0，用已支持的 JSON-RPC CONNECT 完成单聊收发、dispose 清理和 Alice/Bob 验收。',
        'Install exactly v1.1.0 and use supported JSON-RPC CONNECT for person messaging, dispose cleanup, and Alice/Bob acceptance.',
      ),
      publishedPage(
        'javascript/getting-started',
        'Web 快速接入',
        'Web quickstart',
        '精确安装 easyjssdk v2.0.3，通过业务 BFF 与已支持的 JSON-RPC CONNECT 完成 Alice/Bob 在线消息。',
        'Install exactly easyjssdk v2.0.3 and use a product BFF plus supported JSON-RPC CONNECT for Alice/Bob online messaging.',
      ),
    ],
  );
}
