...
function publishedEasySDKGroup(): NavigationGroup {
  return publishedGroup(
    ...
    'WuKongEasySDK',
    'WuKongEasySDK',
    '选择 iOS、Android、Flutter 或 Web 快速接入，并用已验证源码运行官方 example；正式发布包与源码运行凭据保持分离。',
    'Choose an iOS, Android, Flutter, or Web quickstart and run the official examples at verified source revisions while keeping package and source-run evidence separate.',
    [
      publishedPage(
        'examples',
        '运行官方示例',
        'Run Official Examples',
        '启动同一版 WuKongIM，按已验证 revision 跑通四端构建、在线双向消息、断开和清理。',
        'Start the same WuKongIM revision and reproduce four-platform builds, online bidirectional messaging, disconnect, and cleanup.',
      ),
      publishedPage(
        'ios/getting-started',
        'iOS 快速接入',
        'iOS quickstart',
        '精确安装 v1.1.0，或检出已验证 example revision，完成单聊收发、监听清理和 Alice/Bob 验收。',
        'Install exactly v1.1.0 or check out the verified example revision for person messaging, listener cleanup, and Alice/Bob acceptance.',
      ),
      publishedPage(
        'android/getting-started',
        'Android 快速接入',
        'Android quickstart',
        '精确安装 v1.0.4，或检出已验证 example revision，处理单例、单聊收发、清理和 Alice/Bob 验收。',
        'Install exactly v1.0.4 or check out the verified example revision for singleton ownership, person messaging, cleanup, and Alice/Bob acceptance.',
      ),
      publishedPage(
      ...
      publishedPage(
        ...
        'Web 快速接入',
        'Web quickstart',
        '精确安装 easyjssdk v2.0.3，或检出已验证浏览器 example revision，完成 Alice/Bob 在线消息。',
        'Install exactly easyjssdk v2.0.3 or check out the verified browser-example revision for Alice/Bob online messaging.',
      ),
    ],
  );
}
