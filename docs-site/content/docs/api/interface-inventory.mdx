...
| Login info | 1 | 公开已配置的 guest 凭据 | 不注册 |
| Login | 1 | 固定用户登录 | 不注册 |
...
合计 `110`。`POST /manager/slots/leader-transfer-plan` 是只生成计划的读权限路由。`ANY /mcp` 的 MCP Handler 实际只接受 POST，拒绝非空 Origin；其余 Manager 路由使用开放 CORS。完整 method/path 集合由 `lib/api-surface-contracts.ts` 保存并由源码测试校验。

`GET /manager/login` 仅向登录页返回已配置的 `guest` 凭据，并设置 `Cache-Control: no-store`；其他账号保持私有。

`GET /manager/nodes/:node_id/config/toml` 返回版本化、脱敏的启动 TOML 文档，使用相同的 `cluster.node:r` 读取权限。

## Node transport：60 个共享编号
...
| `89` | `slot_message_updates` |
| `90` | `message_update_hint` |

ID `16` 是退役保留项，ID `20` 是删除功能的保留项，均不得复用。默认 Slot proxy 使用已晋升的 `79/80/83/84/85/87/89`；默认身份处理器仅接受 `get_device`，不开放用户读取或扫描；身份元数据使用 `87`，避免原来的 `4` 与 `channel_pull_hint` 冲突。通用但非默认的 Slot Store 仍声明私有 `47/53`，这些仍不能视为稳定目录。这些协议只能进入“不稳定内部合同”页，不能生成伪 OpenAPI 路径。
