...
1. 只允许三个已知 `event` 值，并限制请求体大小与速率。
2. 在自己的事务中写入原始事件、幂等键和持久队列。
3. 持久化成功后尽快返回 `200`，业务逻辑异步执行。
4. `msg.notify` 可用 `event + message_idstr` 去重；`msg.offline` 还要按 UID 去重。
5. Presence 应按业务规则收敛，并允许缺失、重复和乱序，而不是依赖完整过渡序列。
