...
description: Reference all 43 business HTTP operations registered by the current source.
---

Product HTTP provides basic integration and trusted-management operations. The complete OpenAPI contract matches all 43 current `internal/access/api` registrations.
...
The complete contract is authoritative for all 43 runtime entries. The three narrow profiles are reviewed task projections with runnable examples. Generated pages merge those examples and expose caller, node scope, non-atomic writes, and post-200 business checks in an “Operation boundary” callout.
