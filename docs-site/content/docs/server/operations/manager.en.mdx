...
Disabling authentication does not open mutation access. A fresh Web client can enter only the read-only Backups view; it receives no synthetic persisted session, and all writes remain denied.
