...
- Observe ingress, SEND/SENDACK, Channel append, replication/durability, and delivery as separate stages.
...
Retain health freshness, control revision, `blocked_reasons`, tasks, and Slot evidence. A diagnostic recommendation is not permission to force a leader move or remove a node. Scale-in still waits for authoritative `safe_to_remove=true`. Follow [Scaling](/en/server/operations/scaling) for the full procedure.
