...
See [Logs & Observability](/en/server/configuration/observability) for configuration and cost controls. After an alert, follow [Troubleshooting](/en/server/operations/troubleshooting) to collect evidence; never let an alert trigger topology mutations directly.
