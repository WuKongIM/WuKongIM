...
| Gateway | Check TCP, WebSocket, and HTTP forwarding. Keep `proxy_protocol on` when the target binary supports PROXY auto detection; older binaries require removing it. If `proxy_protocol_trusted_cidrs` is nonempty, include the proxy egress in that list. See [PROXY configuration](/en/server/configuration/reference#client-addresses-behind-a-proxy) and run `nginx -t` first. |
...
All target nodes must use matching versions and follow that version’s data-format rollback restrictions.
