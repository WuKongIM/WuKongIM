...
| Gateway | Check TCP, WebSocket, and HTTP forwarding. Keep `proxy_protocol on` only when the target binary supports `proxy_protocol_trusted_cidrs` and its trusted proxy egress CIDRs are configured; otherwise remove it. See [PROXY configuration](/en/server/configuration/reference#client-addresses-behind-a-proxy) and run `nginx -t` first. |
