...
Rehearse against an independent database restored from the complete backup. Explicitly use `--default-character-set=utf8mb4` for MySQL exports and comparison connections, and check non-ASCII identifiers. A reference-only export can replace original text with `?` when its client encoding is wrong, even if the full SQL backup is intact. Before updates, match each original primary key, channel, message ID, and sequence; preserve timestamps and other fields, verify affected-row counts within the transaction, and rehearse with `ROLLBACK` first.

If the final same-generation comparison still finds no corresponding retained IM message, apply the business owner's approved policy. When archiving and removing active rows, preserve complete originals, including bodies, flags, and timestamps; a reference list is insufficient. Archive and remove the active rows in one transaction. An independent v3 business database and cache can retain the original database for the cutover. Bind SQL explicitly to its target database, and inspect `USE`, database-creation statements, views, triggers, routines, and events before restoring under a different database name, so restoration cannot write back to the original.

**Redis is part of the business migration state.** TangSengDaoDao stores login credentials and device state in Redis; an empty cache invalidates existing sessions. After fencing all writers and draining callbacks and read receipts, include a complete Redis snapshot in the same backup generation. Restore into an isolated cache, compare key types, complete values, and absolute expiration times, then verify persistence after restart. When restoring RDB into an AOF target, confirm that RDB has loaded before enabling AOF and testing restart; placing an RDB file beside an active AOF is insufficient. Archive everything before invalidating only derived cursors that must resynchronize, such as `messageExtraVersion:`; preserve credentials, device state, and read/deletion boundaries. Serialized messages in robot queues can also contain `message_seq`: match message ID, original sequence, and channel before mapping it, while preserving event IDs, queue scores, bodies, and expiration. References newer than the rehearsal IM snapshot require the final same-generation comparison and must not be prematurely deleted as orphans.
