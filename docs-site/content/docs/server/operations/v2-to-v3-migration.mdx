...
| 网关 | 核对 TCP、WebSocket 和 HTTP 转发配置；仅在目标二进制支持 PROXY 自动识别，且可选的 `proxy_protocol_trusted_cidrs` 非空时包含代理出口 CIDR 时保留 `proxy_protocol on`，否则移除。参见 [PROXY 配置](/server/configuration/reference#代理后的客户端地址)，并先执行 `nginx -t`。 |
