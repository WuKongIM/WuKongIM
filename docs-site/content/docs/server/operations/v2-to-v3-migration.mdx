...
所有目标节点使用同一配套版本。若现有发布包缺少所需能力，先完成适配和发布版本验证，再安排正式停机；不能把未发布的修复假定为当前安装包已有功能。新版本写入目标数据后，回退须遵守该版本的数据格式约束，不能直接换回旧二进制。
