...
```mermaid
flowchart TD
  Command["Controller command"] --> Proposal["Controller Raft proposal"]
  Proposal --> WAL["Persist WAL"]
  WAL --> Messages["Send Raft messages"] --> Commit["Committed entries"]
  Commit --> Apply["FIFO apply scheduler"]
  Apply --> Batch["Semantic batch + one atomic save"]
  Batch --> State["cluster-state.json + memory snapshot"]
  State --> Index["Persist AppliedRaftIndex"]
```
