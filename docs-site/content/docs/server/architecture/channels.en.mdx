...
```mermaid
flowchart TD
  Batch["AppendBatch (one Channel, request order preserved)"] --> Meta["Resolve and apply current ChannelRuntimeMeta"]
  Meta --> Leader{"Is the local node Channel Leader?"}
  Leader -->|no| Forward["Forward to Leader"] --> Admission
  Leader -->|yes| Admission["Leader reactor admission + epoch / write-fence checks"]
  Admission --> Append["Local durable append"]
  Append --> Replicate["Followers Pull / Apply / ACK"]
  Append --> Progress["Local progress"]
  Replicate --> Progress["ISR progress"]
  Progress --> HW["HW covers append"] --> Future["Append Future succeeds"]
```
