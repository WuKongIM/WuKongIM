...
```mermaid
flowchart TD
  Connect["client CONNECT on node B"] --> Handle["Gateway 创建 SessionHandle"]
  ...
  Register --> Conflict{"存在设备冲突？"}
  Conflict -->|是| Action["owner action: kick / close"] --> Route["CommitRoute"]
  Conflict -->|否| Active
  Route --> Active["online.MarkActive(session)"]
  Active --> Ready["CONNACK / Session 可用于投递"]
```
