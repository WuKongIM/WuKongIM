...
```mermaid
flowchart TB
  subgraph MessagePlane["Message data plane"]
    Client["Client TCP / WebSocket"] --> Gateway --> Usecase["Message use case"]
  ...
  subgraph ControlPlane["Control and metadata plane"]
    Controller["Controller Raft"] --> State["cluster-state.json"] --> Routes["Routing snapshot"]
    Key["Key"] -->|CRC32| Hash["256 physical hash slots"] --> Slot["Logical Slot Raft Group"]
    Routes --> Slot --> Metadata["Distributed metadata and authority routing"]
  end
  Metadata -.->|constrains write authority| Authority
```
