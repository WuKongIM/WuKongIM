...
```mermaid
flowchart TD
  Command["Controller 命令"] --> Proposal["Controller Raft proposal"]
  Proposal --> WAL["WAL 持久化"]
  WAL --> Messages["Raft 消息发送"] --> Commit["committed entries"]
  Commit --> Apply["FIFO apply scheduler"]
  Apply --> Batch["批量语义应用 + 一次原子保存"]
  Batch --> State["cluster-state.json + 内存快照"]
  State --> Index["持久化 AppliedRaftIndex"]
```
