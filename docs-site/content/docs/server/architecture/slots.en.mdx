...
The Slot layer is the distributed metadata store. Users, channel definitions, subscriptions, conversation projections, and `ChannelRuntimeMeta` are partitioned by stable physical hash slot and replicated through logical Slot Raft Groups.
...
- conversation state, unread/read cursors, and active projections;
