...
```mermaid
flowchart TD
  Batch["AppendBatch（同一 Channel，保持请求顺序）"] --> Meta["解析并应用最新 ChannelRuntimeMeta"]
  Meta --> Leader{"本节点是 Channel Leader？"}
  Leader -->|否| Forward["转发到 Leader"] --> Admission
  Leader -->|是| Admission["Leader reactor admission + epoch / write-fence 检查"]
  Admission --> Append["本地 durable append"]
  Append --> Replicate["followers Pull / Apply / ACK"]
  Append --> Progress["本地进度"]
  Replicate --> Progress["ISR progress"]
  Progress --> HW["HW 达到追加序号"] --> Future["Append Future 成功"]
```
