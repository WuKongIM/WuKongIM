...
The preview repository currently publishes `v3.0.0-beta.6` for amd64/x86_64. It is verified on Ubuntu 24.04, Debian 13, Rocky Linux 9, and AlmaLinux 9; RHEL 9 can use the same EL9 repository.
...
Save the administrator password, which is shown only once. Review the node ID, persistent directory, and advertised addresses, then validate the configuration:
