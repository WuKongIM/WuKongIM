...
Do not overwrite a running binary. Validate the target commit and configuration outside production, stage the new binary, then remove one node at a time from traffic, stop it, replace the binary atomically, start it, and wait for `/readyz`. The complete rolling-upgrade, data-compatibility, and rollback matrix remains part of the planned upgrade documentation.
