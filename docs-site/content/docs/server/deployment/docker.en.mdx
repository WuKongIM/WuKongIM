---
title: Docker
description: Start a WuKongIM single-node cluster with docker run, a configuration file, and persistent storage.
---

You only need Docker. This path mounts `wukongim.toml` and a Docker volume directly, with no installer or Compose layer.
...
Replace at least `join_token`, `jwt_secret`, and the administrator password. If clients are not on this host, also replace `127.0.0.1` in both `external_*_addr` values with a reachable hostname or IP address.
...
Docker creates `wukongim-data` automatically on the first run, so no separate volume command is needed.
...
After confirming that the data is no longer needed, run:
...
The last two commands permanently remove message data and deployment credentials.
