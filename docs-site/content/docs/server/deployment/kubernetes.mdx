...
description: 用固定镜像摘要、StatefulSet、稳定节点身份、独立 PVC 和正确探针部署 WuKongIM 集群。
---

本页给出与当前源码配置合同一致的 Kubernetes 参考架构。它适合把已经验证过的 WuKongIM 制品移入现有 Kubernetes 平台，不是仓库内置的 Helm Chart，也不替代容量、安全、备份和故障演练。
