...
这份配置只保留容器启动、API、Manager 认证和可写日志目录所需字段；单节点拓扑、256 Hash Slots、`5100/5200` 网关和其他参数使用运行时默认值。至少替换 `jwt_secret` 和管理员密码。客户端不在本机时，再按实际地址补充 `api.external_tcp_addr` 和 `api.external_ws_addr`。
