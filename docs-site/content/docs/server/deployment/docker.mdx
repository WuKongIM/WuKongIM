...
<Callout type="warn" title="默认用于本机快速体验">
  示例固定使用 `3.0.0-beta.7` 镜像。使用其他版本时直接替换镜像 tag，并先阅读对应版本的升级说明。承载生产流量前，请更换示例凭据并完成[安全配置](/zh/server/configuration/security)和[健康监控](/zh/server/operations/health-and-monitoring)。
</Callout>
...
这份配置只保留容器启动、API、Manager 认证和可写日志目录所需字段；单节点拓扑、256 Hash Slots、`5100/5200` 网关和其他参数使用运行时默认值。至少替换 `jwt_secret` 和管理员密码。客户端不在本机时，再按实际地址补充 `api.external_tcp_addr` 和 `api.external_ws_addr`。全部字段及其默认值见[配置参考](/zh/server/configuration/reference)。
