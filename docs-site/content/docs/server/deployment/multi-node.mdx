...
<Callout type="warn" title="3 台服务器没有备用节点">
  三副本频道的目标副本集合包含 3 个节点。停掉一台后，已有频道在仍满足多数派及运行时条件时可以继续提交；但新频道放置仍要求足够的可用节点，`/readyz` 也检查这一条件，因此其他节点可能返回 `503`。故障演练应分别验证已有频道收发和新频道放置，不能用其中一项代替另一项。如果要求停掉一台后仍能创建三副本频道并满足完整就绪条件，请至少准备 4 台服务器。
</Callout>
