...
`export` opens the source read-only and writes only the output directory. WKDB Import Bundle v1 contains a manifest plus JSONL files for supported user, device, Channel, subscriber, conversation, latest-sequence, and message data, with file row counts and SHA-256 digests.
