...
Version `1.5.5` reports `success` once before synchronization starts and again
after synchronization finishes. Send only after the `success` that follows
`syncMsg` and `syncCompleted`:
