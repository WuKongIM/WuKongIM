...
description: 对照 WuKongEasySDK Android v1.0.4 固定源码学习 Kotlin 接入 API；当前 Product Gateway 支持其 JSON-RPC CONNECT 与在线双向收发。
---
...
这一学习顺序参考[旧版 Android EasySDK 教程](https://wukong.mintlify.app/zh/sdk/easy/android/getting-started)。旧页只提供阅读路径；下面的包坐标、方法签名、版本与限制以固定的 `v1.0.4` 源码和 Maven Central 产物为准。

<Callout type="warn" title="证据边界">
  示例按官方 [WuKongEasySDK-Android](https://github.com/WuKongIM/WuKongEasySDK-Android) `v1.0.4`、源码与发布 revision [`2ab2199a3eb91e6966c6a5d9b6098563e58e3203`](https://github.com/WuKongIM/WuKongEasySDK-Android/commit/2ab2199a3eb91e6966c6a5d9b6098563e58e3203) 校对，发布包可在 [Maven Central](https://central.sonatype.com/artifact/com.githubim/easysdk-android/1.0.4) 与 [GitHub Release](https://github.com/WuKongIM/WuKongEasySDK-Android/releases/tag/v1.0.4) 核对。这是源码级教程，不是本站运行验证，也不代表该版本已经通过当前 WuKongIM v3 的完整兼容性场景。
</Callout>

<Callout type="info" title="v1.0.4 已包含默认静默与脱敏">
  Android `v1.0.4` 的下划线字段与 README 中 JSON 文本字符串 SEND Payload 已由当前服务端兼容；边界同时接受驼峰字段、JSON 对象和 Base64。正式发布版还包含 [PR #3](https://github.com/WuKongIM/WuKongEasySDK-Android/pull/3) 的日志安全修复：`debugLogging` 默认关闭，未知消息、解析失败、RECV 异常等诊断统一经过该总开关，日志与公开模型字符串不会原样输出 Token、Payload 或错误详情。生产构建仍应保持诊断关闭，并用非生产 canary 检查 Logcat、崩溃报告与最终 Release 产物。
</Callout>

## 完成后你会得到什么

- Maven Central 中锁定为 `1.0.4` 的 Android 依赖；
...
Android EasySDK 是进程内单例，而且 `v1.0.4` 初始化后不能用另一份配置再次 `init`。下面示例适合一个进程只登录一个 UID 的最小验证；生产应用若支持账号切换，必须先针对该限制设计并实测完整退出策略。
...
当前服务端已用 Android `v1.0.4` 的 snake_case 字段与 README 对齐的 JSON 文本字符串 SEND Payload 覆盖该闭环。仍应保留服务端 revision、Android tag、设备与网络环境，不要用盲目重试掩盖失败；日志脱敏、离线恢复与生产安全需要另外验收。

## 常见问题

- **`SDK is not initialized`**：严格执行 `init → addEventListener → connect` 顺序。
- **同一 UID 刷新 Token 或路由后提示配置变化**：`v1.0.4` 没有公开重置配置 API；显式停止流程并重启进程，不能继续使用旧 Token 或旧地址。
...
- **关闭 `debugLogging` 后仍在 Logcat 看到原始 JSON 或 `Params`**：先确认 Gradle 依赖解析到 `1.0.4`，清理旧构建，再用非生产 canary 区分 SDK 输出、应用自身日志和第三方网络日志。若 Release 产物仍泄露 canary，应停止上线并提交最小复现。
