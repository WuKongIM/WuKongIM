...
<Callout type="error" title="采用前必须关闭协议与日志阻断项">
  Android `v1.0.3` 的 CONNECT、SEND、RECVACK 与结果模型使用下划线字段。其 README 先把消息编码成 JSON 文本，再将该 `String` 传给 SEND，并非直接传 JSON 对象。当前服务端会在 JSON-RPC 边界同时兼容下划线字段与驼峰字段、JSON 文本字符串、JSON 对象与 Base64 Payload，并按 Android 结果模型返回 snake_case，因此字段与 Payload 不再阻断在线消息。该 tag 的未知消息、解析失败和 RECV 解析异常路径仍会绕过 `debugLogging`，把完整消息或 `Params` 写入 Logcat，其中可能含 Token 或 Payload。协议路径可验收，但原始 tag 不能通过生产日志门禁；上线前仍需升级或修复日志脱敏并检查 Release 产物。
</Callout>
