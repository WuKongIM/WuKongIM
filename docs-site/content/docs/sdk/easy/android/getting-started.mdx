...
description: 对照 WuKongEasySDK Android v1.0.3 固定源码学习 Kotlin 接入 API；当前 Product Gateway 不支持其 JSON-RPC CONNECT。
---

本页沿用旧版教程的“安装 → 初始化 → 监听 → 连接 → 发送”顺序，将其整理成 Android 源码 API 导读和修复后的验收骨架，并补上精确版本、Activity 生命周期与当前 v3 协议边界。
