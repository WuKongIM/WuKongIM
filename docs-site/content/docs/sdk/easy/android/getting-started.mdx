...
<Callout type="info" title="当前 Product Gateway 支持这条连接路径">
  当前 Product Gateway 支持固定 `v1.0.4` 的 JSON-RPC CONNECT 与在线双向收发，兼容 Android snake_case、驼峰字段、JSON 文本字符串、JSON 对象和 Base64 Payload。教程使用 [Maven Central 1.0.4](https://central.sonatype.com/artifact/com.githubim/easysdk-android/1.0.4) 与对应 [Release](https://github.com/WuKongIM/WuKongEasySDK-Android/releases/tag/v1.0.4)。这是源码对齐教程，不是本站运行验证；Android 构建、真机、WSS 与生产 Token 校验仍须在你的环境验收。
</Callout>
