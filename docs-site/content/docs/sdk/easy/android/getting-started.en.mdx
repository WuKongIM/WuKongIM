...
<Callout type="warn" title="Logging fix merged; v1.0.3 does not include it">
  The current server supports Android `v1.0.3` underscore fields and its README's JSON text-string SEND payload; the boundary also accepts camel-case fields, JSON objects, and Base64, so those protocol shapes no longer block online messaging. However, unknown-message, parse-failure, and RECV-error paths in this release still bypass `debugLogging` and can write complete messages or `Params` to Logcat. Official [PR #3](https://github.com/WuKongIM/WuKongEasySDK-Android/pull/3) merged at [`e984c7374a0e11f5d109ad3dbfdea599907735ff`](https://github.com/WuKongIM/WuKongEasySDK-Android/commit/e984c7374a0e11f5d109ad3dbfdea599907735ff), routing diagnostics through a default-silent logger and redacting logs plus public model strings. Maven `1.0.3` does not include that fix. Production users should wait for the next official release. Early evaluation must pin this commit, build the AAR or module independently, and inspect the Release artifact again.
</Callout>
...
- **Raw JSON or `Params` still appears in Logcat with `debugLogging` disabled:** Maven `1.0.3` contains unconditional log paths. [PR #3](https://github.com/WuKongIM/WuKongEasySDK-Android/pull/3) is merged but not released. Stop testing with production tokens or messages; wait for the next official release, or build and verify the exact commit above.
