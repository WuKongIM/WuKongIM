...
<Callout type="info" title="The current Product Gateway supports this connection path">
  The current Product Gateway supports pinned `v1.0.4` JSON-RPC CONNECT and online bidirectional messaging, including Android snake_case, camel-case fields, JSON text strings, JSON objects, and Base64 payloads. The tutorial uses [Maven Central 1.0.4](https://central.sonatype.com/artifact/com.githubim/easysdk-android/1.0.4) and its matching [Release](https://github.com/WuKongIM/WuKongEasySDK-Android/releases/tag/v1.0.4). This source-aligned tutorial is not runtime verification. Your environment still owns the Android build, physical-device run, WSS, and production token evidence.
</Callout>
