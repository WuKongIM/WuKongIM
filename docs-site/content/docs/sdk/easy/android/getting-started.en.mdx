...
description: Install exactly WuKongEasySDK Android 1.0.5 and use contiguous Kotlin code for connection, online messaging, and lifecycle cleanup.
---
...
Use `ws://10.0.2.2:5200` from an Android Emulator, not `localhost`. [Run the Official Examples](/en/sdk/easy/examples) covers server preparation and Alice/Bob acceptance. The source example and Maven `1.0.5` artifact now have separate runtime receipts; keep those evidence classes distinct.
