...
description: Walk through the pinned WuKongEasySDK Android v1.0.3 Kotlin APIs; the current Product Gateway supports JSON-RPC CONNECT and online bidirectional messaging.
---
...
<Callout type="error" title="Close protocol and logging blockers before adoption">
  Android `v1.0.3` uses underscore fields in CONNECT, SEND, RECVACK, and result models. Its README builds the message as JSON text and passes that `String` to SEND rather than passing a JSON object directly. The current server accepts both underscore and camel-case fields plus JSON text strings, JSON objects, and Base64 payloads at the JSON-RPC boundary, then returns snake_case results for Android, so fields and payload no longer block online messaging. Unknown-message, JSON-RPC parse-error, and RECV parse-error paths in this tag still write the complete message or `Params` to Logcat without honoring `debugLogging`; those values can contain a token or payload. The protocol path is testable, but the original tag cannot satisfy a production logging gate. Upgrade or patch log redaction and inspect the Release artifact before production.
</Callout>
