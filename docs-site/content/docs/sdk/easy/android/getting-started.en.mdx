...
description: Walk through the pinned WuKongEasySDK Android v1.0.3 Kotlin APIs; the current Product Gateway does not support its JSON-RPC CONNECT path.
---

This page keeps the legacy install → initialize → listen → connect → send sequence as an Android source-API walkthrough and post-fix acceptance scaffold, while adding an exact version, Activity lifecycle, and the current v3 protocol boundary.
