...
description: Pin WuKongEasySDK Android v1.0.2 and complete installation, connection, event handling, person-Channel send, and cleanup in Kotlin.
---
...
```kotlin
dependencies {
    implementation("com.githubim:easysdk-android:1.0.2")
}
```
...
Android EasySDK is a process singleton, and `v1.0.2` cannot be initialized a second time with another configuration. This example fits a minimal run with one UID per process. A production application that switches accounts must first design and test a complete logout strategy around this limitation.
    ...
    fun connect(bootstrap: IMBootstrap) {
        ...
        if (current == null) {
            ...
            check(
                ...
                "WuKongEasySDK v1.0.2 cannot apply changed identity or configuration; " +
...
Unmodified `v1.0.2` is expected to fail against current v3 at a field, device-type, or payload phase. That is evidence for the compatibility blockers above. Retain the server revision, Android tag, and failing phase instead of hiding it with blind retries. Only a fixed build can produce retained Alice/Bob send and receive evidence.

## Troubleshooting

- **`SDK is not initialized`:** preserve the `init → addEventListener → connect` order.
- **The same UID gets a refreshed token or route and configuration changes:** `v1.0.2` has no public reset API. Stop explicitly and restart the process or upgrade the SDK; do not continue with a stale token or URL.
...
- **CONNECT, SEND, or RECVACK parsing fails:** compare camel-case fields, Base64 payloads, and device values. These are known `v1.0.2` compatibility blockers against current v3.
