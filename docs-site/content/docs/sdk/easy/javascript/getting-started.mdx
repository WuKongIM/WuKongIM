---
title: 5 分钟集成 Web
description: 锁定 easyjssdk v2.0.1，在浏览器应用中通过业务 BFF 完成连接、事件监听、消息发送与页面清理。
---
...
<Callout type="warn" title="证据边界">
  示例按官方 [WuKongEasySDK-JS](https://github.com/WuKongIM/WuKongEasySDK-JS) `v2.0.1`、源码 revision `f13b7fb911fdb2912025e289dcd7749350a54469` 校对。这是源码级教程，不是本站运行验证，也不是现有 `wukongimjssdk@1.3.5` Web 黄金路径的兼容性凭据。
</Callout>

<Callout type="error" title="v2.0.1 日志是生产阻断项">
...
8. 检查生产构建的 Console 与日志采集，确认 Token 和 Payload 不会泄露。`v2.0.1` 未完成这项前不得通过生产门禁。
