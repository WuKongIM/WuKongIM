---
title: Web 快速接入
description: 精确安装 easyjssdk 2.0.3，通过业务 BFF 和可连续复制的 TypeScript 代码完成浏览器在线消息闭环。
---
...
<Callout type="info" title="当前 Product Gateway 支持这条连接路径">
  当前 Product Gateway 支持固定 `v2.0.3` 的 JSON-RPC CONNECT 与在线双向收发，包括省略 `jsonrpc`、Base64 SEND 与对象 RECV。浏览器默认使用 WEB `1`，完整线路值为 APP `0`、WEB `1`、PC `2`。教程安装 [npm 2.0.3](https://www.npmjs.com/package/easyjssdk/v/2.0.3)；官方浏览器 example 已在源码 `a055b3667247333b6b3183249f5d5929673dfd53` 上实测通过。该 revision 比 [v2.0.3 Release](https://github.com/WuKongIM/WuKongEasySDK-JS/releases/tag/v2.0.3) 更新，尚未形成新 npm 版本，因此两份证据不能混用。
</Callout>
...
打开 `http://127.0.0.1:8080/example/`。完整的 WuKongIM 启动、Alice/Bob 准备和地址映射见[运行官方示例](/zh/sdk/easy/examples)。源码运行凭据不等于 npm `2.0.3` 发布包凭据。
...
上述精确源码已在 Chrome 151 中连接同一版 WuKongIM，完成双向消息、主动断开和重连。这个闭环没有覆盖 npm `2.0.3` 产物、离线同步或生产 WSS；如果 Bob 离线，应用不能仅等待 EasySDK 的实时事件，必须用产品设计的持久同步路径恢复消息。
...
- `debugLogging` 默认关闭；生产保持 `false`，用 npm `2.0.3` 实际 bundle 和非生产 canary 检查 Console、错误上报与采集器不含 Token、Payload、原始帧或完整事件对象；
