...
<Callout type="info" title="当前 Product Gateway 支持这条连接路径">
  当前 Product Gateway 支持固定 `v2.0.3` 的 JSON-RPC CONNECT 与在线双向收发，包括省略 `jsonrpc`、Base64 SEND 与对象 RECV。浏览器默认使用 WEB `1`，完整线路值为 APP `0`、WEB `1`、PC `2`。教程使用 [npm 2.0.3](https://www.npmjs.com/package/easyjssdk/v/2.0.3) 与对应 [Release](https://github.com/WuKongIM/WuKongEasySDK-JS/releases/tag/v2.0.3)。这是源码对齐教程，不是本站运行验证；浏览器产物、WSS、BFF 与生产 Token 校验仍须在你的环境验收。
</Callout>
