...
description: Install exactly easyjssdk 2.0.3 and use a product BFF plus contiguous TypeScript code for an online browser messaging loop.
---
...
<Callout type="info" title="The current Product Gateway supports this connection path">
  The current Product Gateway supports pinned `v2.0.3` JSON-RPC CONNECT and online bidirectional messaging, including requests without `jsonrpc`, Base64 SEND, and object RECV. Browsers default to WEB `1`; the full wire set is APP `0`, WEB `1`, and PC `2`. The tutorial installs [npm 2.0.3](https://www.npmjs.com/package/easyjssdk/v/2.0.3); the official browser example passed at source revision `a055b3667247333b6b3183249f5d5929673dfd53`. That revision is ahead of the [v2.0.3 Release](https://github.com/WuKongIM/WuKongEasySDK-JS/releases/tag/v2.0.3) and has not shipped as a new npm version, so the two evidence classes must not be combined.
</Callout>
...
Open `http://127.0.0.1:8080/example/`. [Run the Official Examples](/en/sdk/easy/examples) covers WuKongIM startup, Alice/Bob preparation, and endpoint mapping. A source-run receipt is not an npm `2.0.3` package receipt.
...
- `debugLogging` is off by default. Keep it `false` in production and inspect the actual npm `2.0.3` bundle, Console, error reporting, and collectors with non-production canaries for tokens, payloads, raw frames, or complete event objects.
