...
- **You need an executable Web scenario path:** use the [WuKongIMJSSDK Web tutorial](/en/sdk/javascript), which has the site's protected verification flow, and inspect the current receipt under [Versions & Compatibility](/en/api/compatibility). Do not mix method names between the two SDKs.

## Next

Return to the [WuKongEasySDK overview](/en/sdk/easy), or continue with [Messaging](/en/guide/integration/messaging), [Offline Messages & Push](/en/guide/tutorials/push), and [Integration Acceptance](/en/guide/integration/acceptance).
