...
description: Pin easyjssdk v2.0.1 and use a product BFF to connect, subscribe, send, and clean up a browser application.
---
...
<Callout type="warn" title="Evidence boundary">
  The example is aligned with the official [WuKongEasySDK-JS](https://github.com/WuKongIM/WuKongEasySDK-JS) `v2.0.1` source at revision `f13b7fb911fdb2912025e289dcd7749350a54469`. This is a source-level tutorial, not runtime verification, and it is not a compatibility receipt for the existing `wukongimjssdk@1.3.5` Web golden path.
</Callout>

<Callout type="error" title="v2.0.1 logging blocks production adoption">
...
- An npm dependency pinned to `easyjssdk@2.0.1`.
...
```bash
npm install --save-exact easyjssdk@2.0.1
```
...
8. Inspect production console output and log collection to prove tokens and payloads cannot leak. `v2.0.1` must not pass a production gate before this is fixed.
