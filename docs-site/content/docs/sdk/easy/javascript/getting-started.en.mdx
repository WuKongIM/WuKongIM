...
description: Walk through the pinned easyjssdk v2.0.2 browser APIs; the current Product Gateway does not support its JSON-RPC CONNECT path.
---

This page covers the browser source APIs and a post-fix acceptance scaffold. The browser asks your product BFF for the current user's connection material, then EasyJSSDK attempts a WebSocket JSON-RPC connection. It never holds a Product HTTP management credential directly.
