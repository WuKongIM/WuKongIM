...
<Callout type="info" title="The current Product Gateway supports this connection path">
  The current Product Gateway supports pinned `v2.0.3` JSON-RPC CONNECT and online bidirectional messaging, including requests without `jsonrpc`, Base64 SEND, and object RECV. Browsers default to WEB `1`; the full wire set is APP `0`, WEB `1`, and PC `2`. The tutorial uses [npm 2.0.3](https://www.npmjs.com/package/easyjssdk/v/2.0.3) and its matching [Release](https://github.com/WuKongIM/WuKongEasySDK-JS/releases/tag/v2.0.3). This source-aligned tutorial is not runtime verification. Your environment still owns the browser artifact, WSS, BFF, and production token evidence.
</Callout>
