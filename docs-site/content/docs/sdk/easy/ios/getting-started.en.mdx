...
<Callout type="info" title="The current Product Gateway supports this connection path">
  The current Product Gateway supports pinned `v1.1.0` JSON-RPC CONNECT and online bidirectional messaging, including object and Base64 payloads; camelCase RECVACK carries both `messageId` and `messageSeq`, and device wire values are APP `0`, WEB `1`, and PC `2`. The tutorial installs [CocoaPods 1.1.0](https://cocoapods.org/pods/WuKongEasySDK); the unified iOS/macOS example passed at source revision `40014c16c0becd390c105098d359048901f4d87c`. That revision is ahead of the [v1.1.0 Release](https://github.com/WuKongIM/WuKongEasySDK-iOS/releases/tag/v1.1.0) and has not shipped as a new release.
</Callout>
...
After booting an iOS Simulator, run `./build.sh ios --run`; use `./build.sh macos --run` for macOS. [Run the Official Examples](/en/sdk/easy/examples) covers server preparation and Alice/Bob acceptance. This source-run receipt is not an iOS `1.1.0` package receipt.
...
That exact source ran against the same WuKongIM revision on an iPhone 16 / iOS 18.3 Simulator, completed bidirectional messaging, and rendered content and timestamps correctly. Retain the server and SDK revisions, device, and network evidence; do not attribute the result to the older iOS `1.1.0` artifact. Include physical-device execution, network recovery, offline synchronization, and the UI-ready state in your [Release Checks](/en/guide/integration/acceptance).
