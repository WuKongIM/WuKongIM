...
<Callout type="info" title="当前 Product Gateway 支持这条连接路径">
  当前 Product Gateway 支持固定 `v1.1.0` 的 JSON-RPC CONNECT 与在线双向收发，并兼容对象和 Base64 Payload；camelCase RECVACK 同时携带 `messageId` 与 `messageSeq`，设备线路值为 APP `0`、WEB `1`、PC `2`。教程使用 [CocoaPods 1.1.0](https://cocoapods.org/pods/WuKongEasySDK) 与对应 [Release](https://github.com/WuKongIM/WuKongEasySDK-iOS/releases/tag/v1.1.0)。这是源码对齐教程，不是本站运行验证；iOS 构建、真机、WSS 与生产 Token 校验仍须在你的环境验收。
</Callout>
