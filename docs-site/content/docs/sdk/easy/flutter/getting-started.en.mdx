...
description: Pin WuKongEasySDK Flutter v1.0.3 and complete initialization, connection, messaging, and State lifecycle cleanup.
---
...
<Callout type="warn" title="Evidence boundary">
  The example is aligned with the official [WuKongEasySDK-Flutter](https://github.com/WuKongIM/WuKongEasySDK-Flutter) `v1.0.3` source at revision `7888867a9f22ec22d768dcfe0c6c95b418fcb458`. This is a source-level tutorial, not runtime verification, and it does not mean every Flutter target passed against the same server combination.
</Callout>

<Callout type="error" title="v1.0.3 request logging blocks production adoption">
...
<Callout type="warn" title="The application must decode received payloads">
  `v1.0.3` Base64-encodes SEND payloads as current v3 requires, but leaves a received server Base64 string unchanged in `Message.payload`. The sample explicitly performs Base64 → UTF-8 decoding. Product code then dispatches the JSON by `type`; it must not assume `message.payload` is already a Map.
</Callout>

## What you will have

- A dependency pinned to `1.0.3` in `pubspec.yaml`.
...
7. Inspect system logs and collectors for every Release target to prove that tokens, payloads, and complete JSON-RPC data cannot leak. Unmodified `v1.0.3` must not pass the production gate.
...
- **System logs contain a token or payload:** this is a known `v1.0.3` adoption blocker. Upgrade or patch the logging path and verify the Release artifact before production.
