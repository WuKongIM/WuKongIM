...
This revision is the released `v1.1.0`. [Run the Official Examples](/en/sdk/easy/examples) covers server preparation, device addressing, and Alice/Bob acceptance.
...
The released `v1.1.0` example ran against the same WuKongIM revision on an iOS Simulator and completed bidirectional messaging; all 25 tests and static analysis also passed. This does not prove Android, Web, desktop, application-background recovery, offline message synchronization, or multi-device conversation consistency. Continue with [Messaging](/en/guide/integration/messaging) and the [Release Checks](/en/guide/integration/acceptance).
