...
description: 锁定 WuKongEasySDK Flutter v1.0.3，在 Flutter 应用中完成初始化、连接、消息收发与 State 生命周期清理。
---
...
<Callout type="warn" title="证据边界">
  示例按官方 [WuKongEasySDK-Flutter](https://github.com/WuKongIM/WuKongEasySDK-Flutter) `v1.0.3`、源码 revision `7888867a9f22ec22d768dcfe0c6c95b418fcb458` 校对。这是源码级教程，不是本站运行验证，也不表示所有 Flutter 目标平台都通过了同一服务端组合。
</Callout>

<Callout type="error" title="v1.0.3 请求日志是生产阻断项">
...
<Callout type="warn" title="接收 Payload 需要应用解码">
  `v1.0.3` 会按当前 v3 要求把 SEND Payload 编成 Base64，但收到 RECV 后把服务端 Base64 字符串原样放进 `Message.payload`。示例显式做 Base64 → UTF-8 解码；业务层随后再按 `type` 分派 JSON，不能假设 `message.payload` 已经是 Map。
</Callout>

## 完成后你会得到什么

- `pubspec.yaml` 中锁定为 `1.0.3` 的依赖；
...
7. 检查每个 Release 目标的系统日志与采集服务，证明 Token、Payload 和完整 JSON-RPC 数据不会泄露；`v1.0.3` 原始源码未修复前不得通过生产门禁。
...
- **系统日志出现 Token 或 Payload**：这是 `v1.0.3` 的已知采用阻断项；升级或修复日志路径并验证 Release 产物后再上线。
