...
description: 对照 WuKongEasySDK Flutter v1.0.4 固定源码学习 Dart 接入 API；当前 Product Gateway 不支持其 JSON-RPC CONNECT。
---

本页用一个 `StatefulWidget` 串起 Flutter EasySDK 的源码 API 和修复后验收骨架：取得连接材料、初始化单例、保存事件回调引用、连接、发送，再在 `dispose` 中清理。
