...
<Callout type="warn" title="日志修复已合并，v1.0.4 尚未包含">
  当前发布的 `v1.0.4` 会通过 `developer.log` 无条件记录完整 JSON-RPC 请求与响应，而且没有公开的日志关闭配置。官方 [PR #3](https://github.com/WuKongIM/WuKongEasySDK-Flutter/pull/3) 已在 [`d7758c301e5289ddfa09cd09b6976c2479584b1c`](https://github.com/WuKongIM/WuKongEasySDK-Flutter/commit/d7758c301e5289ddfa09cd09b6976c2479584b1c) 新增默认关闭的 `debugLogging`、可选 `logHandler`，并对诊断与公开模型字符串脱敏，但 pub.dev `1.0.4` 尚未包含该修复。生产环境应等待下一官方版本；提前验证只能将 git dependency 的 `ref` 固定为此精确 commit，自行构建并重新验收每个目标平台。
</Callout>
...
7. 检查每个 Release 目标的系统日志与采集服务，证明 Token、Payload 和完整 JSON-RPC 数据不会泄露；`v1.0.4` 仍不包含已合并的日志修复，必须等待新版或基于上述精确 commit 自行构建后再验收。
...
- **系统日志出现 Token 或 Payload**：这是 pub.dev `1.0.4` 的已知问题；[PR #3](https://github.com/WuKongIM/WuKongEasySDK-Flutter/pull/3) 已合并但尚未发布。等待下一官方版本，或固定到上述 merge commit 自行构建并验证 Release 产物后再上线。
