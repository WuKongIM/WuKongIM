...
This page records two reproducible evidence classes. On August 31, 2026, the source examples in all four official repositories connected to WuKongIM `5676700d2dc966fa6fc9b2f0620a6ae429adad5a`. On September 1, the packages resolved from npm, Maven Central, CocoaPods, and pub.dev connected to the same server source and completed Alice → Bob, Bob → Alice, and disconnect cleanup. The server process-level JSON-RPC E2E also passed at that revision.
...
The Android, iOS, and Flutter hosted jobs—and the released npm `2.0.4` peer shared by each—are recorded in the [released-package acceptance run](https://github.com/WuKongIM/WuKongIM/actions/runs/33466063708).
...
`Safety Automation - EasySDK Released Package Acceptance` resolves the exact four registry versions, builds the current WuKongIM, then uses the released npm package as Bob while Android API 34 and two iOS Simulator jobs complete bidirectional messaging. After the workflow lands, trigger it with:
