...
| Conversations | Recent conversations, read position, unread state, and delete-visibility boundaries | A conversation is a UID-owned projection, not the message log |
...
A durable SEND appends at the Channel authority and waits for quorum durable commit before acknowledgement. Online delivery, conversation projection, webhooks, and post-commit plugins run later; their failure cannot roll back an already committed Channel log.
