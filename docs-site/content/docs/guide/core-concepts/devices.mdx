...
<Callout type="warn" title="多端策略由受信后端控制">
  客户端不能自行选择更高的 Device Level。默认 Gateway 只有在 CONNECT Token 与已存设备记录匹配后，才采用该记录的 Device Level；因此只有受信后端可以写 Token 管理接口。完整的过期、重放与风险策略仍应纳入[上线检查](/zh/guide/integration/acceptance)。
</Callout>
