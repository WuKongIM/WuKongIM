...
<Callout type="warn" title="Production authentication boundary">
  The current v3 Beta default composition exactly matches each CONNECT token against the stored token for the same UID and device category, but this is not a complete account or authorization system. Clients must not call Product HTTP directly; a trusted backend must own issuance, expiry, rotation, and revocation and protect token-management routes.
</Callout>
