...
```mermaid
flowchart TD
  Create["Client creates a stable client_msg_no and selects a Channel and Payload"]
  Check["Check identity, Channel state, and send policy"]
  Commit["Append in Channel order and reach the commit boundary"]
  Ack["Sender receives a send acknowledgement"]
  Online{"Is the receiving device online?"}
  Realtime["Deliver in realtime"]
  Recover["Synchronize missing messages after reconnect"]
  Create --> Check --> Commit
  Commit --> Ack
  Commit -.-> Online
  Online -->|yes| Realtime
...
Continue with [Channel](/en/guide/core-concepts/channels). For integration, read [Messaging](/en/guide/integration/messaging); for commit, replication, and delivery internals, read [Message Flow](/en/server/architecture/message-flow).
