...
<Callout type="warn" title="A trusted backend controls multi-device policy">
  A client cannot promote its own Device Level. The current v3 Beta default Gateway also does not automatically turn stored token and Device Level metadata into a complete verification chain. Implement and test the policy through [Multi-device Sync](/en/guide/core-concepts/devices).
</Callout>

Continue with [Conversation](/en/guide/core-concepts/conversations) and [Multi-device Sync](/en/guide/core-concepts/devices).
