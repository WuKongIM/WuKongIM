...
- **Define synchronization scope:** Messages and conversations are generally UID-scoped, while drafts, download progress, and push tokens belong to a concrete device.
...
| --- | --- |
| Channel messages and message order | Local drafts, scroll position, and download progress |
...
<Callout type="warn" title="A trusted backend controls multi-device policy">
  A client cannot promote its own Device Level. The current v3 Beta default Gateway also does not automatically turn stored token and Device Level metadata into a complete verification chain. Implement and test the policy through [Multi-device Sync](/en/sdk/common-guides/multi-device).
</Callout>

## Relationship to other concepts

- A **User** can own multiple devices, all sharing one UID.
- A **Message** leaves through one concrete connection and may reach the user's other active devices or synchronize later.
- A **Channel** and its history do not belong to one device; a replacement device can recover them when authorized.
- A **Conversation** is generally UID-scoped, while each device owns final presentation and local caches.

Continue with [Conversation](/en/guide/core-concepts/conversations) and [Multi-device Sync](/en/sdk/common-guides/multi-device).
