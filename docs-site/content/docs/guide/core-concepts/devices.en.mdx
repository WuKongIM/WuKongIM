...
```mermaid
stateDiagram-v2
  [*] --> Provisioned: backend assigns UID, device identity, and connection policy
  Provisioned --> Online: connect and associate the Session
  Online --> Disconnected: network loss or connection close
  Disconnected --> Online: create a new Session and recover missing messages
  Online --> Removed: logout or device removal
  Disconnected --> Removed: logout or device removal
  Removed --> [*]: revoke credentials, close connections, remove push bindings
```

<Callout type="warn" title="A trusted backend controls multi-device policy">
  A client cannot promote its own Device Level. The default Gateway adopts the stored Device Level only after the CONNECT token matches that device record, so only a trusted backend may write token-management routes. Keep expiry, replay, and risk policy in the [release checks](/en/guide/integration/acceptance).
</Callout>
