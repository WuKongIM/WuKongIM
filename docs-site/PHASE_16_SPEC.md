...
Publish the two remaining planned pages inside the already-published Product
HTTP group as a source-calibrated, non-exhaustive management Beta subset. Keep
...
Phase 16 publishes matching Chinese and English pages for exactly:

- `/{lang}/api/product-http/channels`
- `/{lang}/api/product-http/conversations`

The pages are generated by Fumadocs OpenAPI from
`contracts/product-http-management.openapi.json`, with reviewed bilingual
narrative supplements in `content/openapi/product-http/`.
...
- bilingual tag-based deterministic Fumadocs generation;
