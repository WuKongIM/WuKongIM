...
func TestStatusModelSnapshotTracksRunStateCountersAndError(t *testing.T) {
	...
	if snapshot.Users != 10 || snapshot.ActiveUsers != 8 || snapshot.Groups != 2 || snapshot.GroupMembers != 3 {
	...
	if snapshot.LastError != "send failed" {
		t.Fatalf("LastError = %q, want send failed", snapshot.LastError)
	}
