...
func printScaleInStatus(w io.Writer, status NodeScaleInStatus) {
	...
	fmt.Fprintf(w, "missing_node=%t join_state_blocked=%t controller_role=%t data_role=%t blocked_by_health=%t blocked_by_runtime_drain=%t blocked_by_control_revision=%t blocked_by_channels=%t unknown_runtime=%t unknown_control_revision=%t unknown_channel_inventory=%t health_report_age_ms=%d health_report_ttl_ms=%d\n",
		...
		status.BlockedByControlRevision || status.BlockedByStaleRevision,
