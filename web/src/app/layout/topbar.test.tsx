...
test("renders brand, top sections, route metadata, and logged-in username", async () => {
  ...
  expect(banner).toHaveClass("bg-background/95", "border-b", "backdrop-blur-xl")
  expect(banner.querySelector("[data-brand-mark]")).toHaveClass("rounded-[10px]")
  expect(banner.querySelector("[data-brand-mark] img")).toHaveAttribute("src", "/logo.png")
  expect(within(banner).getByRole("link", { name: "WuKongIM" })).toHaveAttribute("href", "/cluster/monitor")
