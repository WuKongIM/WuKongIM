...
- Confidence: high for the goroutine buildup fix and medium-high for the three-physical-Slot improvement. The remaining SENDACK p99 is dominated by missing ChannelV2 metadata proposal wait across local and forwarded Slot leaders, runtime append on local channel leaders, and storage commit tails rather than Slot metadata reads, placement/build, final rereads, append admission, reactor mailbox submit, or append batch max-wait. Inside local Slot proposal wait, Raft commit wait and Slot control scheduling are larger than metadata FSM apply/Pebble commit/MarkApplied.
...
- Keep the next optimization focused on Slot Raft commit wait and Slot worker/control scheduling, not `Runtime.Propose` submit or metadata FSM commit. For the node-local ChannelV2 runtime append tail, split the admitted reactor future into durable append, replication/quorum, high-watermark advancement, and completion notification before tuning.
