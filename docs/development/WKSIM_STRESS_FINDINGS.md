...
- For the node-local ChannelV2 runtime append tail, investigate why leader PullHint RPCs can hit remote `channel_not_found` during activation before tuning retry intervals or recovery probe cadence. Start from metadata propagation/lazy follower activation and the window where leaders send hints to followers that have not created or loaded the channel runtime yet.
