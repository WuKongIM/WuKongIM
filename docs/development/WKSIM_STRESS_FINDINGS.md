...
- Confidence: high for the goroutine buildup fix. The remaining SENDACK p99 is dominated by missing ChannelV2 metadata proposal wait across local and forwarded Slot leaders, runtime append on local channel leaders, and storage commit tails rather than Slot metadata reads, placement/build, or final rereads.
...
- Keep the next optimization focused inside the Slot `Future.Wait` path: split Raft commit/apply scheduling, metadata FSM batch apply, and Pebble commit time. `Runtime.Propose` submit is not the bottleneck. Also keep the node-local ChannelV2 runtime append tail on the follow-up list.
