...
## 2026-07-13 three-node 4000 QPS storage and coordinator boundary

- Scenario: `scripts/bench-wukongim-three-nodes-real-qps.sh --qps 4000`, 1000 group channels, 4096 online users, 10 members per channel, synchronous three-replica quorum commit, and a 400ms p99 gate. The host-backed nodes shared one APFS physical store with 9% free space at preflight.
- Attribution: the host baseline delivered 3981.8 actual QPS with zero errors and p99 622.1ms. Physical commit p99 was 89-97ms and logical request p99 was 226-233ms. The exact all-RAM topology delivered 3996.7 actual QPS with zero errors and p99 229.8ms; physical commit p99 fell to 8.5-9.3ms and logical request p99 to 40-48ms. CPU and bounded worker pools were not saturated. The RAM-backed run isolates the software path and is not production durability or capacity evidence.
- Rejected RPC scheduling: concurrent cross-target RPC subgroups reduced queue wait but created transport/store bursts; host p99 rose to 661.2ms and all-RAM p99 to 372.0ms. Stable priority for Pull groups carrying a new durable `AckOffset` also had no repeatable gain. Both experiments were reverted without changing PullHint/Pull, wire, or quorum behavior.
- Rejected commit tuning: increasing the coordinator flush window from 200us to 1ms produced p99 641.7ms and did not materially increase the median grouped request count. Two commit shards produced p99 839.8ms, reduced median requests per physical commit from about 14-15 to 11-13, and raised physical commit p99 from about 89-97ms to 144-162ms. Keep one shard on a shared physical store.
- Boundary: with serial synchronous commits, a request arriving behind a slow fsync can wait for the current physical commit and the next one that contains its own mutation. Within the bounded admission queue and flush window, the default collection has no positive request, record, or byte cap and already folds queued leader-append and follower-apply requests into the next commit. Neither a larger collection window nor concurrent sync commits removes the backing-device tail. The next safe improvement is independent low-latency storage per node, while keeping synchronous durability enabled.
- Evidence: host baseline `docs/development/perf-runs/20260713-001911-three-node-real-qps`, all-RAM baseline `/private/tmp/wukongim-qps-4000-all-ram-20260713`, 1ms window `docs/development/perf-runs/20260713-commit-flush-1ms-three-node-real-qps`, and two shards `docs/development/perf-runs/20260713-commit-shards-2-three-node-real-qps`. RPC scheduling evidence: cross-target host `docs/development/perf-runs/20260713-111111-three-node-real-qps` and all-RAM `/private/tmp/wukongim-qps-4000-target-lanes-all-ram-20260713`; durable-priority host `docs/development/perf-runs/20260713-113029-three-node-real-qps`, all-RAM `/private/tmp/wukongim-qps-4000-durable-priority-all-ram-20260713`, baseline rerun `/private/tmp/wukongim-qps-4000-baseline-rerun-all-ram-20260713`, and priority rerun `/private/tmp/wukongim-qps-4000-durable-priority-rerun2-all-ram-20260713`.
