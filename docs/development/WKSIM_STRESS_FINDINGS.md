...
- For the node-local ChannelV2 runtime append tail, use PullHint result/error-class counters next to classify why `rpc_pull_hint` tasks fail or do not produce accepted follower wakeups before tuning retry intervals or recovery probe cadence.
