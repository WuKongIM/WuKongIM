...
- Keep the next optimization focused on Slot Raft commit wait and Slot worker/control scheduling, not `Runtime.Propose` submit or metadata FSM commit. Also keep the node-local ChannelV2 runtime append tail on the follow-up list.
