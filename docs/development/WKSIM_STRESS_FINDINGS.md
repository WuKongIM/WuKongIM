...
- For the node-local ChannelV2 runtime append tail, verify the PullHint metadata-summary fallback. The previous evidence showed leaders sending hints before follower local metadata reads caught up; the follower should now activate from the leader-carried metadata summary and report `channelv2_pull_hint_receive_meta_hint_ok_count` instead of `meta_resolve/channel_not_found` receive failures.
