...
- The project-local `wukongim-chat-lifecycle` skill is the normal operator interface for start, read-only status, request-scoped stop, and live diagnosis. Only an exact start intent authorizes the aggregate CNY 1,500 paid chain. Deployment seals Manager/Demo credentials to the request's local Codex Ed25519 identity. Every Lease separately generates a Deployment/finalizer Ed25519 identity, seals it to a setup-owned asymmetric wrapping key, validates request/Lease/source/Plan/expiry before use, and deletes the encrypted handoff only after retained zero-inventory proof. The workflows idempotently advance the exact tracking Issue even without a local monitor, and the final 90-day formal-complete Artifact combines released rehearsal evidence, hour-24/hour-72 formal evidence, capacity/recovery, aggregate conservative cost, and final cleanup. Formal Soak latency joins the same five-minute window to complete resource/queue/load-delivery evidence: sustained saturation is a continuing infrastructure warning, clear headroom is product failure, and ambiguity is insufficient evidence. Inspectable runtime failure stops worker traffic and retains a stable at-most-two-hour diagnosis window; terminal Artifact upload retries on the ten-minute finalizer schedule for at most two further hours without extending disk, budget, or expiry safety. Delayed provider-reported billing remains explicitly unresolved rather than being invented from the conservative Quote. An operator stop Artifact blocks later formal procurement before exact orchestration cancellation and bounded finalization.
