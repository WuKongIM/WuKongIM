...
- The GitHub Issue Agent is a GitHub-Actions-only stateless system: signed append-only App-authored Issue checkpoints are its sole state authority; event payloads are wake-up hints, Workers have no GitHub write credential, models cannot author trusted changes/evidence/usage, and only the protected Publisher may create an expected-head verified commit on `agent/issue-<number>`. Codex uses the full-SHA-pinned official Action only to install the CLI, start its loopback Responses proxy, and drop `sudo`; `wkissueagent` receives only a strictly validated bootstrap-home path, while DeepSeek retains its separate Adapter credential. The checked-in rollout is capped at `reproduction`; setup, provider isolation, key rotation, budgets, recovery, and promotion gates live in `docs/agents/issue-agent.md`.
