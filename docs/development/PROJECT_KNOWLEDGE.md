...
- Cloud Lease is the workload-neutral temporary-infrastructure boundary: Quote is read-only; Acquire is inspect-first and idempotent by exact repository/request/Lease/Plan identity; Release succeeds only with an exact zero-inventory proof; and Sweep reconciles expired access plus expired or cleanup-pending Leases. A Request may group sequential stage-specific Leases, and applicable source SHA and bundle digest are typed provenance tags. `wkcloudlease dry-run` exercises the full contract only through the in-memory fake and never accesses a cloud provider. Alibaba Quote is fixed to cn-hangzhou, uses temporary OIDC role credentials and official read-only SDK calls, and selects the cheapest fully evidenced regular PostPaid/NoSpot x86_64 offer that satisfies the use-case Plan. The adapter owns provider capability mapping rather than chat-lifecycle role/count policy, and fails closed unless live EIP quota proves both headroom and direct-ECS retention-fee waiver eligibility. Budget admission still reserves a versioned, expiring, 50-times-published-rate EIP retention allowance for the full Lease plus four cleanup hours.
