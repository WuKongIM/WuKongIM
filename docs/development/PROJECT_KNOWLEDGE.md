...
- A formal chat-lifecycle Lease may start only from an authenticated passing rehearsal transition after the rehearsal selector reaches exact zero inventory. It is a fresh 96-hour Lease bound to the same source, bundle, request, and aggregate cost ledger. One non-restarting systemd wrapper owns the 72-hour Soak, the same-aged-dataset at-most-eight-hour capacity search, and 30-minute recovery at 2,000 SEND/s. A recovered resource/queue breakpoint is `passed_with_capacity_warning/infrastructure_capacity`; latency with explicit headroom is `product_failure`; ambiguous breakpoint evidence is `insufficient_evidence`. Terminal evidence is uploaded before exact Release, and both rehearsal and formal active handoffs participate in the one-paid-scenario startup gate.
