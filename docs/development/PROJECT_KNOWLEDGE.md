...
- Full-scale chat-lifecycle pre-clock readiness allows two hours. The fixed 250,000-new-user/day, 80/20 login stream produces about 3.6 fresh full-sync logins per second while configured session expiry is already active, and the deterministic formal scheduler contract bounds the first synchronized 10,000-online state at 7,000 seconds. The measured rehearsal or formal clock still begins only after all three workers report traffic-ready and the first global grant is accepted; extending this pre-clock bound does not relax the 10,000-online requirement or count initialization as measured traffic.
