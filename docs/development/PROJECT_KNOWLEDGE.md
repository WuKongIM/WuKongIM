...
- Cloud node hosts persist `wukongim.service` cgroup memory peak, effective limit, swap, and cumulative OOM events through node-exporter textfiles; use these before distinguishing a service limit from host capacity or transient product allocation pressure.
