...
- wkbench warmup records typed session-operation failures and continues, while timed measurements continue individual message failures; send, sendack, recv, and recvack wait errors must retain their session and operation code so configured error-rate limits decide the verdict instead of failing the phase, phase cancellation is excluded from error counters, and structural harness failures remain fail-fast.
