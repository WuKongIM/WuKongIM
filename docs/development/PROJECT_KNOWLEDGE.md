...
- The first full-scale chat-lifecycle rehearsal is a distinct two-hour evidence stage over the unchanged formal 10,000-online/250,000-new-user-per-day/2,000-SEND-per-second workload. Its clock starts only after every login completes CONNECT plus a fresh version-0 full conversation sync and the first full coordinator grant is accepted. Remote `wkbench-rehearsal.service` owns execution; the top Action exits after a hashed `run-start.json` handoff, and the scheduled finalizer uploads terminal evidence before releasing until exact zero inventory. After procurement, deployment/readiness repair retains the exact Lease, bundle, and sealed identity while distinct request-trailed protected-`main` control revisions are tried once each; it never repurchases merely for deployment failure and remains bounded by the CNY 1,350 stop, operator stop, a 12-hour rehearsal AutoRelease ceiling, and full stage/release reserve. The ceiling creates repair time but normal PostPaid release remains immediate. A required product/bundle change is terminal for that Lease. Runtime and correctness failures are not retried, and `rehearsal_pass` never means formal pass.
