...
- Agent-directed PR validation uses only fixed suites from `.github/workflows/README.md`; PR/gate-generation-numbered commit statuses are evidence, while the branch-protection gate is a PR-event-bound GitHub Actions check that verifies the exact PR, head SHA, frozen test-merge SHA, gate run, and request run. Cleanup, monitor, and merge-gate verification remain autonomous safety workflows, while phase 1 retains legacy CI and Nightly until branch protection migration is proven.
