...
- The opt-in permission soak is the sustained qualification for that bound: 30 minutes, 4,500 offered messages/s, 5,000 naturally hashed channels, 25 sender/receiver pairs, and public Slot-RPC/goroutine/heap/mutation evidence. The 2026-08-07 local three-process candidate failed after 862,476 completed SEND calls when accumulated `message.send` timeouts filled a gateway async dispatch queue while all nodes remained ready; the short 20,000-message gate stays accepted, but the sustained boundary is unresolved and must not be reported as qualified.
