...
- The first full chat-lifecycle coordinator grant is a pre-clock control round and may use the bounded coordinator round timeout. Only after it crosses every worker does the coordinator freeze `run-start`; all later grant rounds remain capped to the one-second cadence and admit at most 10 milliseconds of platform timestamp quantization while still rejecting future, stale, skipped, or catch-up ticks. Persisted checkpoint windows strip Go's process-local monotonic clock before calculating elapsed wall time, and transport queue capacity is a current-online-session aggregate that may drain to zero rather than a generation-fixed counter.
- Chat-lifecycle retry admission uses the hot SENDACK p99.9 bound for ordinary loaded traffic and the cold p99.9 bound only for the deterministic first person-channel create or an all-node-proven cold reheat. Gateway `SendBatch` coalesces each canonical persistent person directory once and establishes distinct directories through 16 bounded workers before ordered hooks and append admission. Grant failures expose and replay only a closed runtime code; the coordinator retains the lowest-worker classified code when no final report is available.
...
- Full-scale chat-lifecycle pre-clock readiness retains a two-hour safety ceiling, but empty-dataset bootstrap is a distinct fixed global 25-login/second stream rather than the steady growth stream. All three workers still perform real WKProto CONNECT/CONNACK and fresh version-zero full conversation sync with 256 bounded starting slots per worker. Every UTC-aligned second gives the workers immutable 9/8/8 shares, so even subsecond skew across a UTC boundary stays within the aggregate ceiling; missed or unused admission credit is discarded rather than caught up in a burst. Deterministic three-worker churn coverage reaches the first simultaneous 10,000-online state in 421 seconds and enforces that exact result plus a 15-minute scheduler bound. Worker future-work capacity covers each bootstrap forward relationship's maximum initial burst plus one possible lifecycle timer; because that activity cannot be offered before traffic begins, the first owner-applied grant also gives every still-pending item one full eligibility window after the barrier without changing due order. Coordinator-controlled workers remain all-new at their local shares until the first global grant clears bootstrap credit, bucket phase, and the unequal fixed-share attempt ordinals without resetting assigned UIDs, then begins the unchanged 250,000-new-user/day, 80/20 steady stream at about 3.6 total logins per second. The measured rehearsal or formal clock begins only after that grant crosses all three traffic-ready workers; the pre-clock grant uses the ordinary bounded control-round deadline, while later measured grant vectors retain the one-second cadence. Initialization never counts as measured traffic.
