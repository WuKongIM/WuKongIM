...
- The formal chat-lifecycle Soak is a seven-host native-process run: three service/data hosts, three authenticated workers, and one coordinator. It fixes 12 logical Slot Raft Groups over 256 physical hash slots with replicas 3/3, requires at least 1 TB decimal per service data filesystem, stops below 5 percent free, captures a continuing 24-hour qualification, and freezes the final report after worker stop at 72 hours. It has no resume semantics.
