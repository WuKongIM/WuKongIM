...
- Unloaded Channel followers resolve authoritative `ChannelRuntimeMeta` through the centrally allocated `RPCSlotChannelRuntimeMeta` service. Never reuse Channel ACK service ID 3 for Slot proxy reads; that misroutes metadata bytes to the ACK decoder and leaves Channel HW unable to advance even when Slot Raft replicated the row correctly.
