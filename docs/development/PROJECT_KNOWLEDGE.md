...
- The formal chat-lifecycle capacity search runs only after a passing continuous 72-hour Soak checkpoint on the same live aged dataset. It changes the global SEND rate without restarting nodes or workers, refines the first failed boundary, and must prove recovery at 2,000 SEND/s; a clean dataset is comparison evidence only.
