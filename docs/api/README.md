...
```
http://localhost:5001/docs/api/openapi.json
```
...
2. Update the `docs/api/openapi.json` file accordingly
3. Restart the WuKongIM server to see the changes

### Local Development

For local development, ensure the `docs/api/openapi.json` file is present in the project root or docs directory. The documentation endpoint will automatically locate and serve the specification file.
