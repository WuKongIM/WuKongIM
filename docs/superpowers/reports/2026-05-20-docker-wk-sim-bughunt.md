...
| CAP-019 | Recorded | Remaining sustained bottleneck | After PERF-018, active-hint hot-path CPU pressure dropped and clean 0.5/s held zero send/recv errors past 166k messages, but continued 0.5/s later produced send errors; clean 1/s also still hits periodic send/write/sendack timeouts. | Current profiles no longer show one dominant defect; the remaining cost is split across delivery fanout/resolution, gateway writes, transport RPC, and Pebble reads/writes. Active-hint flush deadlines are reduced but not eliminated and correlate with the latest 0.5/s error burst. | Not fixed |
