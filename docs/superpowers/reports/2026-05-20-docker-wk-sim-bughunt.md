...
| 2026-05-21 | `GOWORK=off go test ./internal/bench/workload -run TestAutoRecvAckIdleReadYieldsToForegroundMatcher -count=1` | Red then pass | Experiment 346eac5d proved a background drainer could be forced to yield to a foreground matcher by injecting a short idle read deadline. |
...
| 2026-05-21 | `DOCKER_BUILDKIT=0 docker build --pull=false -t wukongim-dev:local .` | Pass | Rebuilt local image from 346eac5d; image ID `a0a353adb137`. |
| 2026-05-21 | Fresh-image clean 1/s after 346eac5d, `WK_SIM_UID_PREFIX=clean22-autoread-u` | Rejected | The run quickly reached `send_errors=181` by `messages_sent=169889`; polling caught a WKProto stream decode error (`解码包[SEND]失败`) in addition to sendack timeouts, so the short-read-deadline experiment was unsafe. |
| 2026-05-21 | `GOWORK=off go test ./internal/bench/workload -run TestAutoRecvAckIdleReadDoesNotInjectReadDeadline -count=1` | Red then pass | Regression test failed while auto recv-ack injected a deadline into `ReadFrame`, then passed after the drainer stopped shortening socket reads and only yielded after an underlying read returned naturally. |
| 2026-05-21 | `git diff --check`, `GOWORK=off go test ./internal/bench/workload ./internal/bench/worker ./internal/bench/devsim -count=1`, `GOWORK=off go test -race ./internal/bench/workload -run TestAutoRecvAck -count=1`, and `GOWORK=off go test ./...` | Pass | Whitespace, focused bench, race, and full unit verification passed before committing the read-deadline safety fix. |
| 2026-05-21 | `DOCKER_BUILDKIT=0 docker build --pull=false -t wukongim-dev:local .` | Pass | Rebuilt local image from `60f54443`; image ID `2dc9720617cc`. |
| 2026-05-21 | Fresh-image clean 1/s after BUG-034, `WK_SIM_UID_PREFIX=clean23-safe-u` | Fail | The run no longer reproduced the stream decode corruption, but still hit `send_errors=43` by `messages_sent=101201`, `recv_errors=0`; sustained 1/s still needs server-side allocation/latency reduction. |
| 2026-05-21 | `/tmp/wksim-rate1-safeautoread-alloc-20260521-143833` alloc/heap capture | Evidence | The next confirmed server allocation slice was delivery-tag slot assignment indexing: `deliveryTagAssignmentBySlot` allocated about `37.05MB` on node1 while group delivery repeatedly built a full slot-assignment map to read only the required slots. |
| 2026-05-21 | `GOWORK=off go test ./internal/app -run TestCurrentDeliveryTagAssignmentBySlotFiltersCachedAssignmentsToRequiredSlots -count=1` | Red then pass | Regression test failed while cached assignment lookup returned all 128 slots for a two-slot request, then passed after assignment maps were filtered to required slots only. |
| 2026-05-21 | `git diff --check`, `GOWORK=off go test ./internal/app ./internal/usecase/delivery -count=1`, and `GOWORK=off go test -race ./internal/app -run 'Test(CurrentDeliveryTagAssignmentBySlotFiltersCachedAssignmentsToRequiredSlots\|DeliveryTagTopology)' -count=1` | Pass | Whitespace, focused app/delivery, and race verification passed for delivery-tag assignment filtering. |
| 2026-05-21 | `GOWORK=off go test ./...` | Pass | Full unit-test suite passed before committing the delivery-tag assignment allocation fix. |
| 2026-05-21 | `DOCKER_BUILDKIT=0 docker build --pull=false -t wukongim-dev:local .` | Pass | Rebuilt local image from `d0fc7505`; image ID `3a6dcf1376b8`. |
| 2026-05-21 | Fresh-image clean 1/s after PERF-035, `WK_SIM_UID_PREFIX=clean24-tagassign-u` | Pass so far | The run reached `messages_sent=100389`, then `200835`, with `send_errors=0` and `recv_errors=0`, improving over the prior fresh run that had `send_errors=43` by about 101k messages. |
| 2026-05-21 | `/tmp/wksim-rate1-tagassign-alloc-20260521-145134` alloc/heap capture | Evidence | `deliveryTagAssignmentBySlot` dropped from about `37.05MB` on node1 to about `12.51MB`; it no longer appeared in the top allocation list. Remaining allocation is spread across transport encode/decode, replica fetch/state publication, WKProto encode/seal, delivery push, and Pebble. |
| 2026-05-21 | Continued `clean24-tagassign-u` polling and `/tmp/wksim-tagassign-errors-20260521-145547` diagnostics | Fail | Later polling reached `messages_sent=263280`, `send_errors=40`, `recv_errors=0`. Diagnostics still retained only successful events, with slow stages such as `runtime.lane_poll_request_send` up to about `284ms`, `gateway.messages_send` around `129-134ms`, and `message.send_durable` around `126-136ms`. |
