...
### Non-Durable Routing Authority

Non-durable CMD delivery does not have a channel-log owner because it never appends to a channel log. The node that accepts the send coordinates the transient dispatch.

Rules:

- No command-channel runtime metadata refresh is required for non-durable CMD delivery.
- Subscriber resolution still uses the source-channel business authority through the normal subscriber resolver. If that resolver needs authoritative metadata or subscriber pages, it uses the same cluster-safe store/proxy calls as durable delivery.
- Online route discovery and fanout must reach sessions on other nodes through the existing delivery/presence/node RPC path. The accepting node may own the transient delivery actor, but it must not limit delivery to local sessions.
- ACK/retry bindings for transient messages are owned by the accepting node because no durable replay or command-channel owner can repair the message later.
- If cross-node realtime routing cannot accept the transient dispatch, the send returns an infrastructure error instead of pretending the online CMD was delivered.

This keeps the business outcome of legacy `systemcmdonline` without adding a global durable channel or a separate online-CMD cluster owner.
...
- Cross-node online sessions receive non-durable CMD packets through existing delivery/presence/node RPC routing; the implementation must not deliver only to local sessions.
...
- `internal/app`: non-durable CMD routing can target sessions owned by remote nodes without channel-log metadata ownership.
