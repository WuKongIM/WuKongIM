...
- Preserve the PR-event invalidation audit summary and deduplicate overlapping
  invalidation jobs.
...
status API write. The job continues to deduplicate overlapping invalidation
jobs and records a step summary. The generation-bound gate remains the sole
...
3. The invalidation control job records its summary without publishing a
   commit status; its job-scoped concurrency deduplicates overlapping
   invalidation jobs.
4. An authorized Agent publishes a plan and triggers the request.
...
- its script does not publish an unscoped
  `Agent Validation Request / PR #${PR_NUMBER}` status;
...
job as deduplication plus audit summary, not a PR-numbered commit-status writer.
