...
实现基于 `68662c4c8fa7a447ac2864c7352789ae4a5021b7`，首个实现提交为
`35342c5ad`，开发分支为 `codex/transport-performance`。本次完成默认凑批等待、缓冲池及内存准入、
...
- 发送时计算相对预算，接收方从接收时开始计时；调用方仍约束端到端截止时间。
  ...
  导出/恢复为 48h，仓库探测为 5min，Operations MCP 为 1min；显式执行超时
  配置优先。过期或取消会从 FIFO 中移除并释放容量。
...
审查固定比较 `68662c4c8..35342c5ad`，Standards 发现 1 项场景说明维护遗漏，
Spec 发现 3 项兼容性问题，均在合并前处理：为完整备份/恢复保留 48 小时操作
预算，为合法 30 秒 CPU 采样留出初始化及分析时间；新增显式远端 timeout、
canceled、busy、stopped 状态并保留 errors.Is，避免依赖任意错误字符串；同步
领域级 E2E 鉴权前置说明。旧客户端仍将未知非零响应状态视为通用远端错误。
48 小时回滚预算从动作启动独立计时，不重新套用已过期的作业截止时间；不能
声称支持任意长的恢复清理。上述性能数字来自首个实现提交，后续修正集中于
特定长操作和错误路径，未重跑完整性能容量对照。

修正验证：服务端排队/执行预算的真实 TCP 回归已先红后绿；transport、cluster/net
及 channels 的集成和 race 检查通过；在原始提交上运行兼容性探针，四种新增
