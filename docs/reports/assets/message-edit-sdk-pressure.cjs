...
async function main() {
  ...
  const report={complete:false, started_at:new Date().toISOString(), node:process.version, platform:`${os.platform()}/${os.arch()}`, logical_cpus:os.cpus().length,
    ...
    nodes:3, hash_slots:256, physical_slots:8, replicas:3, channels:16, online_clients:32, rounds_per_channel:32};
  ...
  try {
    ...
    const apiSamples=[],visibleSamples=[],slowSamples=[];
    async function client(uid,peer,nodeIndex) {
      ...
      const transport=async(route,body,signal)=>{
        ...
        if(route==='/channel/messageupdates'){feedTimes.push({start,done:performance.now(),status:r.status,versions:data.updates?.map(m=>m.version)});if(feedTimes.length>32)feedTimes.shift()}
      ...
      sdk.chatManager.addMessageUpdateListener(messages=>messages.forEach(m=>{if(rows.has(m.messageID)){rows.set(m.messageID,m);visible.set(`${m.messageID}/${m.contentVersion}`,performance.now())}}));
    ...
    const outcomes=await Promise.allSettled(pairs.map(async({a,b,id})=>{
      a.measure();let original=a.rows.get(id);
      for(let v=1;v<=32;v++) {
        const started=performance.now();original=await a.sdk.chatManager.updateMessage(original,new a.wk.MessageText(`version-${v}`));
    ...
    report.measured_seconds=(performance.now()-start)/1000;clearInterval(sampler);sampler=undefined;
    report.resources_after=sample();report.peak_rss_kib=nodes.map(n=>({node:n.id,rss_kib:n.max_rss_kib}));
    report.bff_edit_roundtrip=quantiles(apiSamples);report.sdk_visible_latency=quantiles(visibleSamples);
    for(const {a,b,id,conversation,unread,timestamp} of pairs) {
      assert.equal(conversation.lastMessage.contentVersion,'32');assert.equal(conversation.unread,unread);assert.equal(conversation.timestamp,timestamp);
      assert.equal(b.rows.get(id).contentVersion,'32');assert.equal(a.errors.length,0,JSON.stringify(a.errors));assert.equal(b.errors.length,0,JSON.stringify(b.errors));
