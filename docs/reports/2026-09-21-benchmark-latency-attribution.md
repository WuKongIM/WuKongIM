...
Apple M4 / darwin arm64、Go 1.25.11（`GOWORK=off` 下由模块 toolchain 选择）、GOMAXPROCS=4，三轮各 1s 的单独微基准：固定桶更新中位数约 3.22ns、0 分配；通过现有 Registry 和四个标签记录约 327.5ns、288B、4 次分配；四线程共享 Registry 的吞吐倒数约 428.5ns/op。并行 ns/op 不是单次延迟，仍存在共享锁竞争。
